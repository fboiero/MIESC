...
Respond with a JSON array."""
    ...
    def _get_rag_context(self, source_code: str) -> str:
        ...
        if "flash" in code_lower and any(
            kw in code_lower for kw in ["vault", "deposit", "share", "borrow"]
        ):
            sections.append(
                "KNOWN FLASH-LOAN VAULT EXPLOITS (check each function explicitly):\n"
                "- First-depositor / share inflation: the first deposit can mint shares at a "
                "manipulable ratio; inspect share = amount * totalShares / totalAssets for the "
                "zero-supply and single-wei edge cases.\n"
                "- Rounding in share↔asset conversion: integer division rounding in the "
                "protocol's disfavor, drainable by repeated small deposit/withdraw cycles.\n"
                "- Borrow/withdraw usable while a flash loan is outstanding: functions that do "
                "not check the flash-loan state can act on temporarily-violated invariants.\n"
                "- Spot-price oracle with no TWAP: collateral or price read from a single-block "
                "manipulable source.\n"
                "- Missing slippage / minimum-out on liquidation or swap paths.\n"
                "- Emergency withdraw without timelock or access control."
            )
