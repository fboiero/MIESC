...
class AgentCapability(Enum):
    ...
    ERC20_TOKEN_COMPATIBILITY_HARDENING = "erc20_token_compatibility_hardening"
    ACCOUNT_ABSTRACTION_USEROP_HARDENING = "account_abstraction_userop_hardening"
    EXTERNAL_CALL_RETURNDATA_HARDENING = "external_call_returndata_hardening"
    FINANCIAL_MATH_PRECISION_HARDENING = "financial_math_precision_hardening"
...
@dataclass(frozen=True)
class ERC20TokenFlowSurface:
    """An ERC20 integration flow where requested and actual token movement may diverge."""

    id: str
    entrypoint: str
    token_call: str = ""
    requested_amount_source: str = ""
    balance_before_source: str = ""
    balance_after_source: str = ""
    credited_amount_source: str = ""
    token_traits: List[str] = field(default_factory=list)
    accounting_fields: List[str] = field(default_factory=list)
    guard: str = ""
    evidence: List[str] = field(default_factory=list)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "id": _safe_label(self.id) or "erc20_token_flow_surface",
            "entrypoint": _safe_label(self.entrypoint) or "unknown",
            "token_call": _safe_text(self.token_call, MAX_AGENT_TEXT_CHARS) or "",
            "requested_amount_source": _safe_text(
                self.requested_amount_source, MAX_AGENT_TEXT_CHARS
            )
            or "",
            "balance_before_source": _safe_text(self.balance_before_source, MAX_AGENT_TEXT_CHARS)
            or "",
            "balance_after_source": _safe_text(self.balance_after_source, MAX_AGENT_TEXT_CHARS)
            or "",
            "credited_amount_source": _safe_text(self.credited_amount_source, MAX_AGENT_TEXT_CHARS)
            or "",
            "token_traits": _safe_text_list(self.token_traits),
            "accounting_fields": _safe_text_list(self.accounting_fields),
            "guard": _safe_text(self.guard, MAX_AGENT_TEXT_CHARS) or "",
            "evidence": _safe_text_list(self.evidence),
        }
...
@dataclass(frozen=True)
class ERC20TokenCompatibilityRisk:
    """A fee-on-transfer, rebasing, non-standard return, or balance-delta risk."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "id": _safe_label(self.id) or "erc20_token_compatibility_risk",
            "category": _safe_label(self.category) or "unknown",
            "affected_surface_id": _safe_label(self.affected_surface_id) or "",
            "severity_hint": _safe_label(self.severity_hint) or "",
            "description": _safe_text(self.description, MAX_AGENT_TEXT_CHARS, allow_multiline=True)
...
@dataclass(frozen=True)
class ERC20TokenCompatibilityHardeningPlan:
    """Provider-neutral plan for ERC20 integration compatibility validation."""

    id: str
    objective: str
    surfaces: List[ERC20TokenFlowSurface] = field(default_factory=list)
    risks: List[ERC20TokenCompatibilityRisk] = field(default_factory=list)
    mitigations: List[str] = field(default_factory=list)
    validation_tests: List[str] = field(default_factory=list)
    recommended_tools: List[str] = field(default_factory=list)
    confidence: float = 0.0
...
@dataclass(frozen=True)
class AccountAbstractionUserOpSurface:
    """An ERC-4337 account, paymaster, factory, or aggregator validation surface."""

    id: str
    component: str
    entrypoint_function: str = ""
    entrypoint_guard: str = ""
    userop_fields: List[str] = field(default_factory=list)
    signature_scheme: str = ""
    nonce_source: str = ""
    prefund_source: str = ""
    paymaster_context: str = ""
    post_op_path: str = ""
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "id": _safe_label(self.id) or "account_abstraction_userop_surface",
            "component": _safe_label(self.component) or "unknown",
            "entrypoint_function": _safe_label(self.entrypoint_function) or "",
            "entrypoint_guard": _safe_text(self.entrypoint_guard, MAX_AGENT_TEXT_CHARS) or "",
            "userop_fields": _safe_text_list(self.userop_fields),
            "signature_scheme": _safe_text(self.signature_scheme, MAX_AGENT_TEXT_CHARS) or "",
            "nonce_source": _safe_text(self.nonce_source, MAX_AGENT_TEXT_CHARS) or "",
            "prefund_source": _safe_text(self.prefund_source, MAX_AGENT_TEXT_CHARS) or "",
            "paymaster_context": _safe_text(self.paymaster_context, MAX_AGENT_TEXT_CHARS) or "",
            "post_op_path": _safe_text(self.post_op_path, MAX_AGENT_TEXT_CHARS) or "",
            "validation_data_source": _safe_text(self.validation_data_source, MAX_AGENT_TEXT_CHARS)
            or "",
            "evidence": _safe_text_list(self.evidence),
        }
...
@dataclass(frozen=True)
class AccountAbstractionUserOpRisk:
    """An ERC-4337 UserOperation validation, gas, replay, or paymaster risk."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "id": _safe_label(self.id) or "account_abstraction_userop_risk",
            "category": _safe_label(self.category) or "unknown",
            "affected_surface_id": _safe_label(self.affected_surface_id) or "",
            "severity_hint": _safe_label(self.severity_hint) or "",
            "description": _safe_text(self.description, MAX_AGENT_TEXT_CHARS, allow_multiline=True)
...
@dataclass(frozen=True)
class AccountAbstractionUserOpHardeningPlan:
    """Provider-neutral plan for ERC-4337 UserOperation validation hardening."""

    id: str
    objective: str
    surfaces: List[AccountAbstractionUserOpSurface] = field(default_factory=list)
    risks: List[AccountAbstractionUserOpRisk] = field(default_factory=list)
    mitigations: List[str] = field(default_factory=list)
    validation_tests: List[str] = field(default_factory=list)
    recommended_tools: List[str] = field(default_factory=list)
    confidence: float = 0.0
...
@dataclass(frozen=True)
class ExternalCallReturndataSurface:
    """A low-level external call surface that consumes untrusted returndata."""
    ...
    call_kind: str = ""
    calldata_source: str = ""
    returndata_source: str = ""
    decode_site: str = ""
    returndata_size_bound: str = ""
    success_guard: str = ""
    trust_boundary: str = ""
    value_forwarding: str = ""
    evidence: List[str] = field(default_factory=list)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "id": _safe_label(self.id) or "external_call_returndata_surface",
            "entrypoint": _safe_label(self.entrypoint) or "unknown",
            "target_source": _safe_text(self.target_source, MAX_AGENT_TEXT_CHARS) or "",
            "call_kind": _safe_label(self.call_kind) or "",
            "calldata_source": _safe_text(self.calldata_source, MAX_AGENT_TEXT_CHARS) or "",
            "returndata_source": _safe_text(self.returndata_source, MAX_AGENT_TEXT_CHARS) or "",
            "decode_site": _safe_text(self.decode_site, MAX_AGENT_TEXT_CHARS) or "",
            "returndata_size_bound": _safe_text(self.returndata_size_bound, MAX_AGENT_TEXT_CHARS)
            or "",
            "success_guard": _safe_text(self.success_guard, MAX_AGENT_TEXT_CHARS) or "",
            "trust_boundary": _safe_text(self.trust_boundary, MAX_AGENT_TEXT_CHARS) or "",
            "value_forwarding": _safe_text(self.value_forwarding, MAX_AGENT_TEXT_CHARS) or "",
            "evidence": _safe_text_list(self.evidence),
        }
...
@dataclass(frozen=True)
class ExternalCallReturndataRisk:
    """A returndata griefing, unsafe decode, or revert-data trust risk."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "id": _safe_label(self.id) or "external_call_returndata_risk",
            "category": _safe_label(self.category) or "unknown",
            "affected_surface_id": _safe_label(self.affected_surface_id) or "",
            "severity_hint": _safe_label(self.severity_hint) or "",
            "description": _safe_text(self.description, MAX_AGENT_TEXT_CHARS, allow_multiline=True)
...
@dataclass(frozen=True)
class ExternalCallReturndataHardeningPlan:
    """Provider-neutral plan for untrusted external-call returndata validation."""

    id: str
    objective: str
    surfaces: List[ExternalCallReturndataSurface] = field(default_factory=list)
    risks: List[ExternalCallReturndataRisk] = field(default_factory=list)
    mitigations: List[str] = field(default_factory=list)
    validation_tests: List[str] = field(default_factory=list)
    recommended_tools: List[str] = field(default_factory=list)
    confidence: float = 0.0
...
@dataclass(frozen=True)
class FinancialMathPrecisionSurface:
    """A financial formula surface whose precision or units need validation."""
    ...
    rounding_direction: str = ""
    operation_order: str = ""
    library_or_helper: str = ""
    unchecked_context: str = ""
    state_fields: List[str] = field(default_factory=list)
    evidence: List[str] = field(default_factory=list)

    def to_dict(self) -> Dict[str, Any]:
        return {
            "id": _safe_label(self.id) or "financial_math_precision_surface",
            "entrypoint": _safe_label(self.entrypoint) or "unknown",
            "formula": _safe_text(self.formula, MAX_AGENT_TEXT_CHARS) or "",
            "value_flow": _safe_text(self.value_flow, MAX_AGENT_TEXT_CHARS) or "",
            "unit_sources": _safe_text_list(self.unit_sources),
            "scale_factor": _safe_text(self.scale_factor, MAX_AGENT_TEXT_CHARS) or "",
            "rounding_direction": _safe_text(self.rounding_direction, MAX_AGENT_TEXT_CHARS) or "",
            "operation_order": _safe_text(self.operation_order, MAX_AGENT_TEXT_CHARS) or "",
            "library_or_helper": _safe_text(self.library_or_helper, MAX_AGENT_TEXT_CHARS) or "",
            "unchecked_context": _safe_text(
                self.unchecked_context, MAX_AGENT_TEXT_CHARS, allow_multiline=True
            )
            or "",
            "state_fields": _safe_text_list(self.state_fields),
            "evidence": _safe_text_list(self.evidence),
        }


@dataclass(frozen=True)
class FinancialMathPrecisionRisk:
    """A precision-loss, rounding, scale, or intermediate arithmetic risk."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "id": _safe_label(self.id) or "financial_math_precision_risk",
            "category": _safe_label(self.category) or "unknown",
            "affected_surface_id": _safe_label(self.affected_surface_id) or "",
            "severity_hint": _safe_label(self.severity_hint) or "",
            "description": _safe_text(self.description, MAX_AGENT_TEXT_CHARS, allow_multiline=True)
...
@dataclass(frozen=True)
class FinancialMathPrecisionHardeningPlan:
    """Provider-neutral plan for financial math precision validation."""

    id: str
    objective: str
    surfaces: List[FinancialMathPrecisionSurface] = field(default_factory=list)
    risks: List[FinancialMathPrecisionRisk] = field(default_factory=list)
    mitigations: List[str] = field(default_factory=list)
    validation_tests: List[str] = field(default_factory=list)
    recommended_tools: List[str] = field(default_factory=list)
    confidence: float = 0.0
...
class ERC20TokenCompatibilityHardeningAgent:
    """Builds ERC20 compatibility/accounting hardening plans through a replaceable provider."""
    ...
    def build_task(
        ...
        *,
        ...
        safe_findings = _json_safe_list(findings or [], max_items=25)
        safe_token_summary = _json_safe_mapping(token_summary or {})
        prompt = _erc20_token_compatibility_hardening_prompt(
            safe_source,
            safe_findings,
            safe_token_summary,
        )
        return ReasoningTask(
            ...
            objective=(
                "Build provider-neutral ERC20 token compatibility and accounting "
                "hardening plans for fee-on-transfer, rebasing, non-standard return, "
                "and actual-received amount risks."
            ),
            prompt=prompt,
            inputs={
                "contract_path": _safe_text(contract_path or "", MAX_AGENT_TEXT_CHARS),
            ...
            output_schema={
                "erc20_token_compatibility_hardening_plans": [
                    {
                        "id": "short_stable_identifier",
                        "objective": "ERC20 compatibility accounting hardening goal",
                        "surfaces": [
                            {
                                ...
                                "balance_before_source": "token.balanceOf(address(this)) before transfer",
                                "balance_after_source": "token.balanceOf(address(this)) after transfer",
                                "credited_amount_source": "amount or balance delta",
                                "token_traits": [
                                    "fee_on_transfer",
                                    "rebasing",
                                    "missing_return",
                                ...
                                "guard": "SafeERC20 and balance-delta accounting",
                                "evidence": ["specific ERC20 integration signal"],
                            }
                        ],
                        "risks": [
                            {
                                "id": "requested_amount_credit",
                                "category": "fee_on_transfer_misaccounting|rebasing_balance_drift|nonstandard_return|zero_transfer_revert|blacklist_pause_incompatibility|allowance_race",
                                "affected_surface_id": "deposit_transfer_surface",
                                "severity_hint": "high|medium|low",
                                "description": "why requested and actual token movement can diverge",
                                "evidence": ["specific code signal"],
                        ...
                        "mitigations": [
                            ...
                            "support rebasing-safe internal accounting",
                        ],
                        "validation_tests": [
                            "fee-on-transfer token cannot inflate credited deposits"
                        ],
                        "recommended_tools": ["foundry", "slither", "stateful fuzzing"],
                        "confidence": 0.0,
                        "evidence": ["specific ERC20 integration signal"],
    ...
    def plan(
        ...
        *,
        ...
        findings: Optional[List[Mapping[str, Any]]] = None,
        token_summary: Optional[Mapping[str, Any]] = None,
    ) -> List[ERC20TokenCompatibilityHardeningPlan]:
        task = self.build_task(
            source_code,
            contract_path=contract_path,
            findings=findings,
        ...
        result = self.provider.complete_json(task)
        return parse_erc20_token_compatibility_hardening_plans(result.data)


class AccountAbstractionUserOpHardeningAgent:
    """Builds ERC-4337 UserOperation hardening plans through a replaceable provider."""
    ...
    def build_task(
        ...
        *,
        contract_path: Optional[str] = None,
        findings: Optional[List[Mapping[str, Any]]] = None,
        account_summary: Optional[Mapping[str, Any]] = None,
    ) -> ReasoningTask:
        safe_source = _bounded_source(source_code, self.policy.max_source_chars)
        safe_findings = _json_safe_list(findings or [], max_items=25)
        safe_account_summary = _json_safe_mapping(account_summary or {})
        prompt = _account_abstraction_userop_hardening_prompt(
            safe_source,
            safe_findings,
        ...
        return ReasoningTask(
            ...
            objective=(
                "Build provider-neutral ERC-4337 account abstraction and "
                "UserOperation hardening plans for EntryPoint, account, factory, "
                "aggregator, and paymaster validation risks."
            ),
            prompt=prompt,
            inputs={
                "contract_path": _safe_text(contract_path or "", MAX_AGENT_TEXT_CHARS),
                "source_chars": len(safe_source),
                "findings": safe_findings,
                "account_summary": safe_account_summary,
            },
            output_schema={
                "account_abstraction_userop_hardening_plans": [
                    {
                        "id": "short_stable_identifier",
                        "objective": "ERC-4337 UserOperation validation hardening goal",
                        "surfaces": [
                            {
                                "id": "validate_userop_surface",
                                "component": "account|paymaster|factory|aggregator|entrypoint",
                                "entrypoint_function": "validateUserOp",
                                "entrypoint_guard": "require(msg.sender == entryPoint)",
                                "userop_fields": [
                                    "sender",
                                    "nonce",
                                    "initCode",
                                ...
                                "signature_scheme": "userOpHash bound to EntryPoint and chain",
                                "nonce_source": "nonce consumed per sender/key",
                                "prefund_source": "missingAccountFunds or paymaster deposit",
                                "paymaster_context": "context returned by validatePaymasterUserOp",
                                "post_op_path": "postOp charges sponsored operation",
                                "validation_data_source": "aggregator and time range packed in validationData",
                                "evidence": ["specific ERC-4337 validation signal"],
                            }
                        ],
                        "risks": [
                            {
                                "id": "missing_entrypoint_guard",
                                "category": "entrypoint_guard|userop_replay|nonce_scope|signature_scope|prefund_accounting|paymaster_drain|postop_revert|validation_data_bounds|aggregator_trust|initcode_factory",
                                "affected_surface_id": "validate_userop_surface",
                                "severity_hint": "high|medium|low",
                                "description": "why UserOperation validation or sponsorship can be bypassed",
                                "evidence": ["specific code signal"],
                                "recommended_check": "direct validateUserOp call must fail unless msg.sender is EntryPoint",
                            }
                        ],
                        "mitigations": [
                            "gate validation and execution to the trusted EntryPoint",
                            "bind signatures to userOpHash, chain, EntryPoint, sender, nonce, and calldata",
                            "make paymaster postOp charging non-bypassable and bounded",
                        ],
                        "validation_tests": [
                            "direct validateUserOp call from non-EntryPoint reverts"
                        ],
                        "recommended_tools": ["foundry", "erc4337 simulation", "stateful fuzzing"],
                        "confidence": 0.0,
                        "evidence": ["specific account abstraction signal"],
                    }
                ]
            },
            policy=self.policy,
    ...
    def plan(
        ...
        *,
        ...
        findings: Optional[List[Mapping[str, Any]]] = None,
        account_summary: Optional[Mapping[str, Any]] = None,
    ) -> List[AccountAbstractionUserOpHardeningPlan]:
        task = self.build_task(
            source_code,
            contract_path=contract_path,
            findings=findings,
        ...
        result = self.provider.complete_json(task)
        return parse_account_abstraction_userop_hardening_plans(result.data)


class ExternalCallReturndataHardeningAgent:
    """Builds external-call returndata hardening plans through a replaceable provider."""
    ...
    def build_task(
        ...
        *,
        contract_path: Optional[str] = None,
        findings: Optional[List[Mapping[str, Any]]] = None,
        call_summary: Optional[Mapping[str, Any]] = None,
    ) -> ReasoningTask:
        safe_source = _bounded_source(source_code, self.policy.max_source_chars)
        safe_findings = _json_safe_list(findings or [], max_items=25)
        safe_call_summary = _json_safe_mapping(call_summary or {})
        prompt = _external_call_returndata_hardening_prompt(
            safe_source,
            safe_findings,
            safe_call_summary,
        )
        return ReasoningTask(
            ...
            objective=(
                "Build provider-neutral external-call returndata hardening plans for "
                "untrusted low-level call/staticcall return data, unsafe decode, "
                "unbounded returndata copy, and revert-data trust risks."
            ),
            prompt=prompt,
            inputs={
                "contract_path": _safe_text(contract_path or "", MAX_AGENT_TEXT_CHARS),
                "source_chars": len(safe_source),
                "findings": safe_findings,
                "call_summary": safe_call_summary,
            },
            output_schema={
                "external_call_returndata_hardening_plans": [
                    {
                        "id": "short_stable_identifier",
                        "objective": "external call returndata hardening goal",
                        "surfaces": [
                            {
                                "id": "low_level_staticcall_decode",
                                "entrypoint": "quote",
                                "target_source": "adapter address controlled by registry",
                                "call_kind": "call|staticcall",
                                "calldata_source": "abi.encodeWithSelector(...)",
                                "returndata_source": "bytes memory data from low-level call",
                                "decode_site": "abi.decode(data, (uint256))",
                                "returndata_size_bound": "data.length == 32",
                                "success_guard": "require(success)",
                                "trust_boundary": "untrusted target",
                                "value_forwarding": "none or msg.value forwarded",
                                "evidence": ["specific low-level call signal"],
                            }
                        ],
                        "risks": [
                            {
                                "id": "malformed_returndata_decode",
                                "category": "unbounded_returndata_copy|unsafe_abi_decode|malformed_returndata_dos|revert_data_bubbling_dos|untrusted_staticcall_decode|unchecked_success|returndata_ignored|success_without_code_check",
                                "affected_surface_id": "low_level_staticcall_decode",
                                "severity_hint": "high|medium|low",
                                "description": "why untrusted returndata can break caller assumptions",
                                "evidence": ["specific code signal"],
                                "recommended_check": "malicious target returns malformed or huge returndata",
                            }
                        ],
                        "mitigations": [
                            "check success before decoding returndata",
                            "bound returndata length before abi.decode",
                            "avoid trusting bubbled revert data from untrusted targets",
                        ],
                        "validation_tests": [
                            "malicious target with huge returndata cannot DoS caller"
                        ],
                        "recommended_tools": ["foundry", "slither", "stateful fuzzing"],
                        "confidence": 0.0,
                        "evidence": ["specific external-call returndata signal"],
                    }
                ]
            },
            policy=self.policy,
    ...
    def plan(
        ...
        *,
        ...
        findings: Optional[List[Mapping[str, Any]]] = None,
        call_summary: Optional[Mapping[str, Any]] = None,
    ) -> List[ExternalCallReturndataHardeningPlan]:
        task = self.build_task(
            source_code,
            contract_path=contract_path,
            findings=findings,
        ...
        result = self.provider.complete_json(task)
        return parse_external_call_returndata_hardening_plans(result.data)


class FinancialMathPrecisionHardeningAgent:
    """Builds financial math precision hardening plans through a replaceable provider."""
    ...
    def build_task(
        ...
        *,
        contract_path: Optional[str] = None,
        findings: Optional[List[Mapping[str, Any]]] = None,
        math_summary: Optional[Mapping[str, Any]] = None,
    ) -> ReasoningTask:
        safe_source = _bounded_source(source_code, self.policy.max_source_chars)
        safe_findings = _json_safe_list(findings or [], max_items=25)
        safe_math_summary = _json_safe_mapping(math_summary or {})
        prompt = _financial_math_precision_hardening_prompt(
            safe_source,
            safe_findings,
            safe_math_summary,
        )
        return ReasoningTask(
            ...
            objective=(
                "Build provider-neutral financial math precision hardening plans for "
                "formula ordering, fixed-point scale, rounding direction, unit "
                "conversion, and accumulator precision risks."
            ),
            prompt=prompt,
            inputs={
                "contract_path": _safe_text(contract_path or "", MAX_AGENT_TEXT_CHARS),
                "source_chars": len(safe_source),
                "findings": safe_findings,
                "math_summary": safe_math_summary,
            },
            output_schema={
                "financial_math_precision_hardening_plans": [
                    {
                        "id": "short_stable_identifier",
                        "objective": "financial math precision hardening goal",
                        "surfaces": [
                            {
                                ...
                                "unit_sources": ["amount:token", "totalShares:shares"],
                                "scale_factor": "1e18",
                                "rounding_direction": "down",
                                "operation_order": "division_before_multiplication",
                                "library_or_helper": "Math.mulDiv",
                                "unchecked_context": "none",
                                "state_fields": ["accRewardPerShare"],
                                "evidence": ["specific formula signal"],
                            }
                        ],
                        "risks": [
                            {
                                "id": "division_before_multiplication",
                                "category": "division_before_multiplication|rounding_to_zero|scale_mismatch|basis_point_denominator|muldiv_precision|unchecked_financial_arithmetic|rounding_direction_bias|accumulator_drift|unit_conversion_loss|overflow_in_intermediate",
                                "affected_surface_id": "reward_formula_surface",
                                "severity_hint": "high|medium|low",
                                "description": "why precision loss or rounding changes value flow",
                                "evidence": ["specific code signal"],
                                "recommended_check": "small amount does not round reward to zero",
                            }
                        ],
                        "mitigations": [
                            "use full-precision mulDiv",
                            "make rounding direction explicit",
                            "normalize units before arithmetic",
                        ],
                        "validation_tests": ["small and large values preserve expected precision"],
                        "recommended_tools": ["foundry", "slither", "stateful fuzzing"],
                        "confidence": 0.0,
                        "evidence": ["specific financial math signal"],
                    }
                ]
            },
            policy=self.policy,
    ...
    def plan(
        ...
        *,
        ...
        findings: Optional[List[Mapping[str, Any]]] = None,
        math_summary: Optional[Mapping[str, Any]] = None,
    ) -> List[FinancialMathPrecisionHardeningPlan]:
        task = self.build_task(
            source_code,
            contract_path=contract_path,
            findings=findings,
        ...
        result = self.provider.complete_json(task)
        return parse_financial_math_precision_hardening_plans(result.data)
...
def parse_erc20_token_compatibility_hardening_plans(
    data: Any,
) -> List[ERC20TokenCompatibilityHardeningPlan]:
    """Parse provider output into bounded ERC20 compatibility/accounting plans."""
    if isinstance(data, str):
        try:
            data = json.loads(data)
        except json.JSONDecodeError:
                ...
                data.get(
                    ...
                    data.get(
                        ...
                        data.get(
                            ...
                            data.get("safe_erc20_compatibility_plans", []),
                        ),
                    ),
                ),
            ),
    ...
    plans: List[ERC20TokenCompatibilityHardeningPlan] = []
    for raw in raw_items[:MAX_AGENT_ITEMS]:
        if not isinstance(raw, Mapping):
            continue
        objective = _safe_text(raw.get("objective"), MAX_AGENT_TEXT_CHARS, allow_multiline=True)
        surfaces = _parse_erc20_token_flow_surfaces(raw.get("surfaces"))
        risks = _parse_erc20_token_compatibility_risks(raw.get("risks"))
        mitigations = _safe_text_list(raw.get("mitigations"))
        validation_tests = _safe_text_list(raw.get("validation_tests"))
        if not objective:
            continue
        if not surfaces and not risks and not mitigations and not validation_tests:
            continue
        plans.append(
            ERC20TokenCompatibilityHardeningPlan(
                id=_safe_label(raw.get("id")) or _derive_invariant_id(objective),
                objective=objective,
                surfaces=surfaces,
                risks=risks,
...
def _parse_erc20_token_flow_surfaces(value: Any) -> List[ERC20TokenFlowSurface]:
    if not isinstance(value, list):
        return []
    surfaces: List[ERC20TokenFlowSurface] = []
    for raw in value[:MAX_AGENT_ITEMS]:
        if not isinstance(raw, Mapping):
            continue
        surface_id = _safe_label(raw.get("id"))
        ...
        if not surface_id or not entrypoint:
            continue
        surfaces.append(
            ERC20TokenFlowSurface(
                id=surface_id,
                entrypoint=entrypoint,
                token_call=_safe_text(raw.get("token_call"), MAX_AGENT_TEXT_CHARS) or "",
                requested_amount_source=_safe_text(
                    raw.get("requested_amount_source"), MAX_AGENT_TEXT_CHARS
                )
                or "",
                balance_before_source=_safe_text(
                    raw.get("balance_before_source"), MAX_AGENT_TEXT_CHARS
                )
                or "",
                balance_after_source=_safe_text(
                    raw.get("balance_after_source"), MAX_AGENT_TEXT_CHARS
                )
                or "",
                credited_amount_source=_safe_text(
                    raw.get("credited_amount_source"), MAX_AGENT_TEXT_CHARS
                )
                or "",
                token_traits=_safe_text_list(raw.get("token_traits")),
                accounting_fields=_safe_text_list(raw.get("accounting_fields")),
                guard=_safe_text(raw.get("guard"), MAX_AGENT_TEXT_CHARS) or "",
                evidence=_safe_text_list(raw.get("evidence")),
            )
...
def _parse_erc20_token_compatibility_risks(
    value: Any,
) -> List[ERC20TokenCompatibilityRisk]:
    if not isinstance(value, list):
        return []
    risks: List[ERC20TokenCompatibilityRisk] = []
    for raw in value[:MAX_AGENT_ITEMS]:
        if not isinstance(raw, Mapping):
            continue
        risk_id = _safe_label(raw.get("id"))
...
def parse_account_abstraction_userop_hardening_plans(
    data: Any,
) -> List[AccountAbstractionUserOpHardeningPlan]:
    """Parse provider output into bounded ERC-4337 UserOperation hardening plans."""
    if isinstance(data, str):
        try:
            data = json.loads(data)
        except json.JSONDecodeError:
        ...
        data.get(
            ...
            data.get(
                ...
                data.get(
                    ...
                    data.get("paymaster_validation_hardening_plans", []),
                ),
            ),
        ),
    )
    ...
    plans: List[AccountAbstractionUserOpHardeningPlan] = []
    for raw in raw_items[:MAX_AGENT_ITEMS]:
        if not isinstance(raw, Mapping):
            continue
        objective = _safe_text(raw.get("objective"), MAX_AGENT_TEXT_CHARS, allow_multiline=True)
        surfaces = _parse_account_abstraction_userop_surfaces(raw.get("surfaces"))
        risks = _parse_account_abstraction_userop_risks(raw.get("risks"))
        mitigations = _safe_text_list(raw.get("mitigations"))
        validation_tests = _safe_text_list(raw.get("validation_tests"))
        if not objective:
            continue
        if not surfaces and not risks and not mitigations and not validation_tests:
            continue
        plans.append(
            AccountAbstractionUserOpHardeningPlan(
                id=_safe_label(raw.get("id")) or _derive_invariant_id(objective),
                objective=objective,
                surfaces=surfaces,
                risks=risks,
...
def _parse_account_abstraction_userop_surfaces(
    value: Any,
) -> List[AccountAbstractionUserOpSurface]:
    if not isinstance(value, list):
        return []
    surfaces: List[AccountAbstractionUserOpSurface] = []
    for raw in value[:MAX_AGENT_ITEMS]:
        if not isinstance(raw, Mapping):
            continue
        surface_id = _safe_label(raw.get("id"))
        component = _safe_label(raw.get("component"))
        if not surface_id or not component:
            continue
        surfaces.append(
            AccountAbstractionUserOpSurface(
                id=surface_id,
                component=component,
                entrypoint_function=_safe_label(raw.get("entrypoint_function")) or "",
                entrypoint_guard=_safe_text(raw.get("entrypoint_guard"), MAX_AGENT_TEXT_CHARS)
                or "",
                userop_fields=_safe_text_list(raw.get("userop_fields")),
                signature_scheme=_safe_text(raw.get("signature_scheme"), MAX_AGENT_TEXT_CHARS)
                or "",
                nonce_source=_safe_text(raw.get("nonce_source"), MAX_AGENT_TEXT_CHARS) or "",
                prefund_source=_safe_text(raw.get("prefund_source"), MAX_AGENT_TEXT_CHARS) or "",
                paymaster_context=_safe_text(raw.get("paymaster_context"), MAX_AGENT_TEXT_CHARS)
                or "",
                post_op_path=_safe_text(raw.get("post_op_path"), MAX_AGENT_TEXT_CHARS) or "",
                validation_data_source=_safe_text(
                    raw.get("validation_data_source"), MAX_AGENT_TEXT_CHARS
                )
                or "",
                evidence=_safe_text_list(raw.get("evidence")),
            )
...
def _parse_account_abstraction_userop_risks(
    value: Any,
) -> List[AccountAbstractionUserOpRisk]:
    if not isinstance(value, list):
        return []
    risks: List[AccountAbstractionUserOpRisk] = []
    for raw in value[:MAX_AGENT_ITEMS]:
        if not isinstance(raw, Mapping):
            continue
        risk_id = _safe_label(raw.get("id"))
...
def parse_external_call_returndata_hardening_plans(
    data: Any,
) -> List[ExternalCallReturndataHardeningPlan]:
    """Parse provider output into bounded external-call returndata plans."""
    if isinstance(data, str):
        try:
            data = json.loads(data)
        except json.JSONDecodeError:
        ...
        data.get(
            ...
            data.get(
                ...
                data.get(
                    ...
                    data.get("external_call_return_value_hardening_plans", []),
                ),
            ),
        ),
    )
    ...
    plans: List[ExternalCallReturndataHardeningPlan] = []
    for raw in raw_items[:MAX_AGENT_ITEMS]:
        if not isinstance(raw, Mapping):
            continue
        objective = _safe_text(raw.get("objective"), MAX_AGENT_TEXT_CHARS, allow_multiline=True)
        surfaces = _parse_external_call_returndata_surfaces(raw.get("surfaces"))
        risks = _parse_external_call_returndata_risks(raw.get("risks"))
        mitigations = _safe_text_list(raw.get("mitigations"))
        validation_tests = _safe_text_list(raw.get("validation_tests"))
        if not objective:
            continue
...
def _parse_external_call_returndata_surfaces(
    value: Any,
) -> List[ExternalCallReturndataSurface]:
    if not isinstance(value, list):
        return []
    surfaces: List[ExternalCallReturndataSurface] = []
    for raw in value[:MAX_AGENT_ITEMS]:
        if not isinstance(raw, Mapping):
            continue
        surface_id = _safe_label(raw.get("id"))
        ...
        surfaces.append(
            ExternalCallReturndataSurface(
                ...
                call_kind=_safe_label(raw.get("call_kind")) or "",
                calldata_source=_safe_text(raw.get("calldata_source"), MAX_AGENT_TEXT_CHARS) or "",
                returndata_source=_safe_text(raw.get("returndata_source"), MAX_AGENT_TEXT_CHARS)
                or "",
                decode_site=_safe_text(raw.get("decode_site"), MAX_AGENT_TEXT_CHARS) or "",
                returndata_size_bound=_safe_text(
                    raw.get("returndata_size_bound"), MAX_AGENT_TEXT_CHARS
                )
                or "",
                success_guard=_safe_text(raw.get("success_guard"), MAX_AGENT_TEXT_CHARS) or "",
                trust_boundary=_safe_text(raw.get("trust_boundary"), MAX_AGENT_TEXT_CHARS) or "",
...
def _parse_external_call_returndata_risks(
    value: Any,
) -> List[ExternalCallReturndataRisk]:
    if not isinstance(value, list):
        return []
    risks: List[ExternalCallReturndataRisk] = []
    for raw in value[:MAX_AGENT_ITEMS]:
        if not isinstance(raw, Mapping):
            continue
        risk_id = _safe_label(raw.get("id"))
...
def parse_financial_math_precision_hardening_plans(
    data: Any,
) -> List[FinancialMathPrecisionHardeningPlan]:
    """Parse provider output into bounded financial math precision plans."""
    if isinstance(data, str):
        try:
            data = json.loads(data)
        except json.JSONDecodeError:
            ...
            data.get(
                ...
                data.get(
                    ...
                    data.get(
                        ...
                        data.get("division_before_multiplication_hardening_plans", []),
                    ),
                ),
            ),
        ),
    ...
    plans: List[FinancialMathPrecisionHardeningPlan] = []
    for raw in raw_items[:MAX_AGENT_ITEMS]:
        if not isinstance(raw, Mapping):
            continue
        objective = _safe_text(raw.get("objective"), MAX_AGENT_TEXT_CHARS, allow_multiline=True)
        surfaces = _parse_financial_math_precision_surfaces(raw.get("surfaces"))
        risks = _parse_financial_math_precision_risks(raw.get("risks"))
        mitigations = _safe_text_list(raw.get("mitigations"))
        validation_tests = _safe_text_list(raw.get("validation_tests"))
        if not objective:
            continue
...
def _parse_financial_math_precision_surfaces(
    value: Any,
) -> List[FinancialMathPrecisionSurface]:
    if not isinstance(value, list):
        return []
    surfaces: List[FinancialMathPrecisionSurface] = []
    for raw in value[:MAX_AGENT_ITEMS]:
        if not isinstance(raw, Mapping):
            continue
        surface_id = _safe_label(raw.get("id"))
        ...
        surfaces.append(
            FinancialMathPrecisionSurface(
                id=surface_id,
                entrypoint=entrypoint,
                formula=_safe_text(raw.get("formula"), MAX_AGENT_TEXT_CHARS) or "",
                value_flow=_safe_text(raw.get("value_flow"), MAX_AGENT_TEXT_CHARS) or "",
                unit_sources=_safe_text_list(raw.get("unit_sources")),
                scale_factor=_safe_text(raw.get("scale_factor"), MAX_AGENT_TEXT_CHARS) or "",
                rounding_direction=_safe_text(raw.get("rounding_direction"), MAX_AGENT_TEXT_CHARS)
                or "",
                operation_order=_safe_text(raw.get("operation_order"), MAX_AGENT_TEXT_CHARS) or "",
                library_or_helper=_safe_text(raw.get("library_or_helper"), MAX_AGENT_TEXT_CHARS)
                or "",
                unchecked_context=_safe_text(
                    raw.get("unchecked_context"), MAX_AGENT_TEXT_CHARS, allow_multiline=True
                )
                or "",
                state_fields=_safe_text_list(raw.get("state_fields")),
                evidence=_safe_text_list(raw.get("evidence")),
            )
        )
    return surfaces
...
def _parse_financial_math_precision_risks(
    value: Any,
) -> List[FinancialMathPrecisionRisk]:
    if not isinstance(value, list):
        return []
    risks: List[FinancialMathPrecisionRisk] = []
    for raw in value[:MAX_AGENT_ITEMS]:
        if not isinstance(raw, Mapping):
            continue
        risk_id = _safe_label(raw.get("id"))
...
def _erc20_token_compatibility_hardening_prompt(
    source_code: str,
    findings: List[Any],
    token_summary: Dict[str, Any],
) -> str:
...
Build ERC20 token compatibility and accounting hardening plans for defensive
Solidity review. Focus on integrations that assume ERC20 transfers move exactly
the requested amount or behave like vanilla ERC20 tokens, including
fee-on-transfer or deflationary tokens, rebasing tokens, non-standard return
values, blacklist/pause behavior, zero-value transfer reverts, allowance quirks,
and accounting that credits requested amount instead of actual balance delta.
    ...
    {{
      ...
      "surfaces": [
        {{
          ...
          "balance_after_source": "token.balanceOf(address(this)) after transfer",
          "credited_amount_source": "amount or balance delta",
          "token_traits": ["fee_on_transfer", "rebasing", "missing_return"],
          "accounting_fields": ["balances", "totalDeposits"],
          "guard": "SafeERC20 and balance-delta accounting",
          "evidence": ["specific code signal"]
        }}
      ...
      "mitigations": ["use SafeERC20", "credit balance delta", "support rebasing-safe internal accounting"],
      "validation_tests": ["fee-on-transfer token cannot inflate credited deposits"],
      "recommended_tools": ["foundry", "slither", "stateful fuzzing"],
      "confidence": 0.0,
      "evidence": ["specific ERC20 integration signal"]
    }}
...
Constraints:
- Do not duplicate ERC4626 vault inflation hardening; focus on general ERC20
  integration compatibility unless the vault bug is specifically actual-received
  token accounting.
- Do not duplicate token hook reentrancy hardening; callbacks are out of scope
  unless they also show requested-vs-actual token accounting drift.
- Do not reduce this to generic unchecked return value triage. SafeERC20 is one
  mitigation, but the core issue is compatibility with tokens whose balances,
  fees, returns, pauses, or transfer semantics differ from vanilla ERC20.
- Require concrete evidence for token call, requested amount, credited amount,
...
- Prefer SafeERC20, balance-delta crediting, internal accounting for rebasing,
  explicit token allowlists, and tests with fee-on-transfer/rebasing/mock
  non-standard ERC20 tokens.

Existing findings, if any:
{findings_json}

Token compatibility summary, if any:
{summary_json}
...
def _account_abstraction_userop_hardening_prompt(
    source_code: str,
    findings: List[Any],
    account_summary: Dict[str, Any],
) -> str:
    findings_json = json.dumps(findings[:25], sort_keys=True)
    summary_json = json.dumps(account_summary, sort_keys=True)
    prompt = f"""You are an interchangeable security reasoning agent.

Build account abstraction UserOperation hardening plans for defensive Solidity
review. Focus on ERC-4337-style smart accounts, paymasters, factories,
aggregators, and EntryPoint integrations where validation, sponsorship, replay,
prefund, postOp, nonce, time-range, or initCode assumptions can fail.
    ...
    {{
      ...
      "surfaces": [
        {{
          ...
          "entrypoint_function": "validateUserOp",
          "entrypoint_guard": "require(msg.sender == entryPoint)",
          "userop_fields": ["sender", "nonce", "callData", "paymasterAndData", "signature"],
          "signature_scheme": "userOpHash bound to EntryPoint and chain",
          "nonce_source": "nonce consumed per sender/key",
          "prefund_source": "missingAccountFunds or paymaster deposit",
          "paymaster_context": "context returned by validatePaymasterUserOp",
...
Constraints:
- Do not duplicate signature-domain hardening. Only include signature evidence
  when it is bound to ERC-4337 userOpHash, EntryPoint, sender, nonce,
  paymasterAndData, aggregator, or validationData semantics.
- Do not duplicate CREATE2 deployment hardening. initCode/factory evidence is in
  scope only when it affects UserOperation validation or sender creation.
- Do not duplicate cross-chain message hardening. Multichain replay is in scope
  only when the same UserOperation authorization can be accepted across chains
  because chain, EntryPoint, sender, or nonce scope is missing.
- Do not duplicate ERC20 token compatibility hardening. Paymaster ERC20 charging
  is in scope only when sponsorship accounting, postOp, or prefund collection can
  be bypassed.
- Require concrete evidence for the EntryPoint guard, validation function,
  UserOperation fields, nonce/signature scope, prefund handling, paymaster
  context, postOp behavior, and validationData bounds when present.

Existing findings, if any:
{findings_json}

Account abstraction summary, if any:
{summary_json}
...
def _external_call_returndata_hardening_prompt(
    source_code: str,
    findings: List[Any],
    call_summary: Dict[str, Any],
) -> str:
    findings_json = json.dumps(findings[:25], sort_keys=True)
    summary_json = json.dumps(call_summary, sort_keys=True)
    prompt = f"""You are an interchangeable security reasoning agent.

Build external call returndata hardening plans for defensive Solidity review.
Focus on low-level call/staticcall surfaces where a contract consumes bytes
returned by an untrusted target, decodes malformed returndata, copies or bubbles
unbounded returndata, assumes revert data is authentic, or updates state after a
failed call because success and return-data shape are not checked together.
...
{{
  "external_call_returndata_hardening_plans": [
    {{
      ...
      "validation_tests": ["malicious target with huge returndata cannot DoS caller"],
      "recommended_tools": ["foundry", "slither", "stateful fuzzing"],
      "confidence": 0.0,
      "evidence": ["specific external-call returndata signal"]
    }}
...
Constraints:
- Do not duplicate delegatecall storage-aliasing hardening. Exclude delegatecall
  storage corruption; only mention delegatecall when the risk is strictly
  returndata size, decode, or bubbling.
- Do not duplicate token hook reentrancy hardening. Callback-specific ERC777,
  ERC1363, or ERC1820 flows are out of scope unless the issue is returned bytes.
- Do not duplicate ERC20 token compatibility hardening. Missing optional token
  returns, fee-on-transfer, rebasing, and SafeERC20 behavior are out of scope
  unless the contract manually decodes low-level returned bytes.
- Do not reduce this to generic unchecked-call triage. The value of this plan is
  the shape, size, provenance, and use of returndata after success is known.
- Do not duplicate ABI packed hash collision hardening. abi.encodePacked
  ambiguity is out of scope; abi.decode(returndata, ...) is in scope.
- Require concrete evidence for call kind, target trust boundary, success guard,
  returndata source, decode or bubbling path, size bound, and state update or
  downstream assumption affected by malformed returndata.

Existing findings, if any:
{findings_json}

External call summary, if any:
{summary_json}
...
def _financial_math_precision_hardening_prompt(
    source_code: str,
    findings: List[Any],
    math_summary: Dict[str, Any],
) -> str:
    findings_json = json.dumps(findings[:25], sort_keys=True)
    summary_json = json.dumps(math_summary, sort_keys=True)
    prompt = f"""You are an interchangeable security reasoning agent.

Build financial math precision hardening plans for defensive Solidity review.
Focus on formulas where operation order, fixed-point scale, unit conversion,
rounding direction, intermediate overflow, unchecked arithmetic, or accumulator
drift can change balances, rewards, fees, shares, debt, collateral, AMM quotes,
liquidation thresholds, or protocol solvency.
    ...
    {{
      ...
      "surfaces": [
        {{
          ...
          "formula": "reward = amount / totalShares * rewardRate",
          "value_flow": "reward distribution",
          "unit_sources": ["amount:token", "totalShares:shares", "rewardRate:1e18"],
          "scale_factor": "1e18",
          "rounding_direction": "down",
          "operation_order": "division_before_multiplication",
          "library_or_helper": "Math.mulDiv",
...
Constraints:
- Do not duplicate ERC4626 vault inflation hardening. Only include vaults when
  the issue is a general financial formula precision problem, not donation,
  first-depositor, preview, empty-vault, or lifecycle share inflation behavior.
- Do not duplicate oracle feed hardening. Feed decimals are in scope only when
  a formula scale mismatch is the bug; feed freshness, source trust, bounds,
  and sequencer guards are out of scope.
- Do not duplicate economic attack simulation. This capability validates formula
  surfaces and precision assumptions, not full capital strategy simulation.
- Do not reduce this to Solidity version or overflow triage. Solidity 0.8
  checked arithmetic does not prevent precision/rounding loss.
- Require concrete evidence for formula, operation order, scale factor or units,
  rounding direction, affected state fields, value flow, and unchecked context
  when present.
- Prefer full-precision mulDiv, explicit rounding direction, consistent
  units/decimals, multiply-before-divide when safe, denominator guards, and
  boundary tests with tiny, large, and adversarial values.

Existing findings, if any:
{findings_json}

Financial math summary, if any:
{summary_json}
