...
class SimplifiedCorrelationAPI:
    ...
    def __init__(
        ...
        self.engine = SmartCorrelationEngine(
...
def evaluate_with_correlation(
    ...
    return {
        ...
        'cross_validated': sum(1 for f in correlated_findings if f.get('is_cross_validated', False)),
        'avg_confidence': sum(f['confidence']['final'] for f in correlated_findings) / max(len(correlated_findings), 1),
    }
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.6,
        help='Umbral de probabilidad FP (0.0-1.0)'
    )
    parser.add_argument(
        '--max-contracts',
        type=int,
        default=None,
        help='Máximo de contratos a evaluar'
    )
    parser.add_argument(
        '--output',
        type=str,
    ...
    if args.output:
        ...
        output_path = PROJECT_ROOT / "benchmarks" / "results" / f"correlation_eval_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
