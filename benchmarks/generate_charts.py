...
# Add value labels
for bar, val in zip(bars1, recalls, strict=False):
    ax1.text(val + 2, bar.get_y() + bar.get_height() / 2, f"{val:.1f}%", va="center", fontsize=10)
...
# Add value labels
for bar, val in zip(bars2, lit_recalls, strict=False):
    ax2.text(
...
thesis_fig = (
    ...
    / "thesis_generator"
    / "figuras"
    / "Figura 31 SmartBugs Evaluation Results.png"
)
