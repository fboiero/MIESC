...
def analyze_contract(contract_path: str):
    ...
    print(
        f"      Transaction sequence: deposit(1 ETH) → withdraw() → fallback() loops{Colors.ENDC}"
    )
    print()
    ...
    print("      All 3 layers detected the SAME critical reentrancy issue.")
    print("      This is NOT a false positive. Priority: IMMEDIATE FIX REQUIRED")
    print()
    ...
    time.sleep(0.5)
    print("    • OWASP SC Top 10: SC01 - Reentrancy Attacks")
    print("    • SWC Registry: SWC-107 - Reentrancy")
    print("    • CWE: CWE-841 - Improper Enforcement of Behavioral Workflow")
    print("    • NIST SSDF: PW.7 - Review and/or analyze code")
    print()
    ...
    print("    1. Implement ReentrancyGuard from OpenZeppelin")
    print("    2. Move state changes before external calls")
    print("    3. Add test coverage for reentrancy scenarios")
    print("    4. Re-run MIESC to verify fixes")
    print()
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            f"{Colors.YELLOW}Example: python simple_demo.py examples/contracts/EtherStore.sol{Colors.ENDC}\n"
        )
