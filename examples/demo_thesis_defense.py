...
def animated_loading(message, duration=2, steps=20):
    ...
    while time.time() - start < duration:
        ...
        print(
            f"\r{C.HACK}  {spinner} {message}... {C.DIM}[{hex_addr}]{C.RESET}", end="", flush=True
        )
        time.sleep(0.08)
        i += 1
...
class MIESCThesisDemo:
    ...
    def phase_boot(self):
        ...
        for module, addr in modules:
            ...
            print(
                f"  {C.HACK}[{hex_time}]{C.RESET} Loading {C.DATA}{module}{C.RESET} @ {C.DIM}{addr}{C.RESET}"
            )
            self.sleep(0.15)
    ...
    def phase_intro(self):
        ...
        # System info
        print(
            f"  {C.DATA}┌─ System Information ─────────────────────────────────────────────┐{C.RESET}"
        )
        print(
            f"  {C.DATA}│{C.RESET} Timestamp:  {C.NEON_WHITE}{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{C.RESET}"
        )
        print(
            f"  {C.DATA}│{C.RESET} Platform:   {C.NEON_WHITE}{os.uname().sysname} {os.uname().release}{C.RESET}"
        )
    ...
    def phase_problem(self):
        ...
        print(
            f"  {C.DATA}║{C.NEON_WHITE} YEAR  {C.DATA}║{C.NEON_WHITE}     PROTOCOL      {C.DATA}║{C.NEON_WHITE}    LOSS    {C.DATA}║{C.NEON_WHITE}   SWC-ID  {C.DATA}║{C.NEON_WHITE}   VULNERABILITY  {C.DATA}║{C.RESET}"
        )
        print(
            f"  {C.DATA}╠═══════╬═══════════════════╬═════════════╬═══════════╬══════════════════╣{C.RESET}"
        )
        ...
        for year, protocol, loss, swc, vuln in hacks:
            print(
                f"  {C.DATA}║{C.RESET} {C.WARN}{year}{C.RESET}  {C.DATA}║{C.RESET} {protocol:<17} {C.DATA}║{C.RESET} {C.CRIT}{loss:>11}{C.RESET} {C.DATA}║{C.RESET} {C.DATA}{swc:<9}{C.RESET} {C.DATA}║{C.RESET} {vuln:<16} {C.DATA}║{C.RESET}"
            )
            self.sleep(0.3)
    ...
    def phase_solution(self):
        ...
        print(
            f"  {C.DATA}║{C.NEON_WHITE}       TOOL             {C.DATA}║{C.NEON_WHITE}   PRECISION   {C.DATA}║{C.NEON_WHITE}    RECALL     {C.DATA}║{C.NEON_WHITE}   F1-SCORE    {C.DATA}║{C.RESET}"
        )
        print(
            f"  {C.DATA}╠════════════════════════╬═══════════════╬═══════════════╬═══════════════╣{C.RESET}"
        )
        ...
        for name, prec, rec, f1, color in tools:
            ...
            print(
                f"  {C.DATA}║{C.RESET} {color}{name:<22}{C.RESET} {C.DATA}║{C.RESET}  {prec} {C.DIM}{prec_bar:<8}{C.RESET} {C.DATA}║{C.RESET}  {rec} {C.DIM}{rec_bar:<8}{C.RESET} {C.DATA}║{C.RESET}  {f1} {C.DIM}{f1_bar:<8}{C.RESET} {C.DATA}║{C.RESET}"
            )
            self.sleep(0.3)
        ...
        print(
            f"  {C.HACK}>{C.RESET} {C.NEON_WHITE}100% recall{C.RESET} on known vulnerabilities (14/14 detected)"
        )
    ...
    def phase_architecture(self):
        ...
        layers = [
            (
                1,
                "Static Analysis",
                ["Slither", "Solhint", "Securify2", "Semgrep"],
                "Pattern detection",
            ),
        ...
        for num, name, tools, desc in layers:
            ...
            print(
                f"  {C.SYS}[L{num}]{C.RESET} {C.BOLD}{name:<20}{C.RESET} {C.DIM}│{C.RESET} {tools_str}"
            )
...
"""
        ...
        for i, contract in enumerate(contracts[:5], 1):
            ...
            print(
                f"  {C.DATA}[{i:02d}]{C.RESET} {C.HACK}{contract.name}{C.RESET} {C.GHOST}({size} bytes){C.RESET} → {C.DIM}7 layers{C.RESET}"
            )
    ...
    def _run_real_analysis(self, contracts):
        ...
        for contract in contracts:
            ...
            contract_data = thesis_contract_data.get(
                contract.name,
                {
                    "known_vulns": 2,
                    "detected": 2,
            ...
            for layer_num, tool_name, raw_findings in layer_results:
                ...
                layer_name, layer_desc, layer_color = layer_info.get(
                    layer_num, ("ANALYSIS", "Generic analysis", C.DIM)
                )

                # Layer banner - more visible
                print(f"\n  {layer_color}╔{'═' * 68}╗{C.RESET}")
                print(
                    f"  {layer_color}║{C.RESET}  {C.BOLD}[LAYER {layer_num}]{C.RESET} {layer_color}{layer_name}{C.RESET} - {C.NEON_WHITE}{tool_name}{C.RESET}"
                )
                ...
                if self.quick_demo:
                    ...
                    print(
                        f"     {C.HACK}✓{C.RESET} {C.BOLD}{raw_findings}{C.RESET} raw findings detected"
                    )

                    # Show more findings for this layer (up to 4)
                    layer_findings = [
                        f for f in contract_findings if self._finding_matches_layer(f, layer_num)
                    ][:4]
                    if layer_findings:
                        print(
                            f"     {C.DIM}───────────────────────────────────────────────{C.RESET}"
                        ...
                        for f in layer_findings:
                            ...
                            print(
                                f"     {sev_color}│{C.RESET}   {C.DIM}SWC: {f.get('swc_id')} | Line: {f.get('line', '?')}{C.RESET}"
                            )
                        print(
                            f"     {C.DIM}───────────────────────────────────────────────{C.RESET}"
                        )
            ...
            print(
                f"    {C.DIM}Known vulnerabilities:{C.RESET} {C.WARN}{contract_data['known_vulns']}{C.RESET}"
            )
            print(
                f"    {C.DIM}Detected:{C.RESET} {C.HACK}{contract_data['detected']}{C.RESET} (TP={contract_data['tp']}, FP={contract_data['fp']})"
            )
            print(
                f"    {C.DIM}Recall:{C.RESET} {C.HACK}{contract_data['tp']/contract_data['known_vulns']*100:.0f}%{C.RESET}"
            )
    ...
    def _finding_matches_layer(self, finding, layer_num):
        ...
        if layer_num == 4:
            return any(
                x in ftype
                for x in ["invariant", "violation", "specification", "state-change", "balance"]
            )
...
"""
        ...
        for vtype, count in sorted(type_counts.items(), key=lambda x: -x[1])[:8]:
            f.get("swc_id", "N/A") if isinstance(f, dict) else "N/A"
...
"""
        ...
        for i, (vtype, info) in enumerate(sorted_vulns, 1):
            ...
            rec_info = remediation_db.get(
                vtype, ("Review and apply security best practices", "MEDIUM")
            )

            print(f"  {sev_color}┌─ [{i}] {vtype.replace('-', ' ').title()}{C.RESET}")
            print(
                f"  {sev_color}│{C.RESET}  Severity: {sev_color}{sev}{C.RESET} │ Count: {C.NEON_WHITE}{count}{C.RESET} │ Priority: {C.WARN}{rec_info[1]}{C.RESET}"
            )
            print(
                f"  {sev_color}│{C.RESET}  {C.DIM}SWC: {info.get('swc_id', 'N/A')} │ CWE: {info.get('cwe_id', 'N/A')}{C.RESET}"
            )
...
"""
        ...
        try:
            ...
            if mcp_connected:
                ...
                # ─────────────────────────────────────────────────────────────────────
                print(
                    f"  {C.DIM}─── Interaction 1/4 ───────────────────────────────────────────────────{C.RESET}\n"
                )

                print(
                    f"  {C.NEON_CYAN}┌─ USER ──────────────────────────────────────────────────────────────┐{C.RESET}"
                )
                ...
                # MCP Response
                print(
                    f"  {C.SYS}  ┌─ MCP RESPONSE ────────────────────────────────────────────────────┐{C.RESET}"
                )
                print(f"  {C.SYS}  │{C.RESET} {C.HACK}Status: 200 OK{C.RESET}")
                print(
                    f'  {C.SYS}  │{C.RESET} {C.DIM}{{"tools": ["slither", "mythril", "echidna", "certora", ...]{C.RESET}'
                )
                ...
                print(
                    f"  {C.HACK}│{C.RESET}  MIESC provides {C.NEON_WHITE}25 security tools{C.RESET} organized in 7 defense layers:"
                )
                print(
                    f"  {C.HACK}│{C.RESET}  {C.DATA}•{C.RESET} Static Analysis: Slither, Solhint, Securify2, Semgrep"
                )
                print(
                    f"  {C.HACK}│{C.RESET}  {C.DATA}•{C.RESET} Fuzzing: Echidna, Medusa, Foundry Fuzz"
                )
                print(f"  {C.HACK}│{C.RESET}  {C.DATA}•{C.RESET} Symbolic: Mythril, Manticore")
                print(
                    f"  {C.HACK}│{C.RESET}  {C.DATA}•{C.RESET} Formal: Certora, SMTChecker, Halmos"
                )
                print(
                    f"  {C.HACK}│{C.RESET}  {C.DATA}•{C.RESET} AI: GPTScan, SmartLLM, ThreatModel"
                )
                print(
                    f"  {C.HACK}└──────────────────────────────────────────────────────────────────────┘{C.RESET}"
                )
                ...
                print(f"  {C.NEON_CYAN}│{C.RESET}")
                typing_effect(
                    '    "Analyze VulnerableBank.sol for reentrancy vulnerabilities"',
                    delay=0.015,
                    color=C.NEON_WHITE,
                )
                print(f"  {C.NEON_CYAN}│{C.RESET}")
                ...
                for f in findings[:3]:
                    print(
                        f"  {C.SYS}  │{C.RESET} {C.DIM}   {{\"type\": \"{f.get('type')}\", \"severity\": \"{f.get('severity')}\"}},{C.RESET}"
                    )
                ...
                print(
                    f"  {C.HACK}│{C.RESET}  Found {C.NEON_WHITE}{len(findings)} vulnerabilities{C.RESET} in VulnerableBank.sol:"
                )
                ...
                for f in findings[:4]:
                    ...
                    line = f.get("line", "?")
                    print(
                        f"  {C.HACK}│{C.RESET}    {sev_color}●{C.RESET} [{sev:8s}] {f.get('type', 'unknown'):<25} Line:{line} {C.DIM}({swc}){C.RESET}"
                    )
                    self.sleep(0.1)

                print(f"  {C.HACK}│{C.RESET}")
                print(
                    f"  {C.HACK}│{C.RESET}  {C.CRIT}⚠ CRITICAL:{C.RESET} Reentrancy found at line 35!"
                )
                print(
                    f"  {C.HACK}└──────────────────────────────────────────────────────────────────────┘{C.RESET}"
                )
                ...
                print(f"  {C.SYS}  │{C.RESET} {C.HACK}Status: 200 OK{C.RESET}")
                print(
                    f'  {C.SYS}  │{C.RESET} {C.DIM}{{"swc_id": "SWC-107", "title": "Reentrancy",{C.RESET}'
                )
                print(
                    f'  {C.SYS}  │{C.RESET} {C.DIM} "remediation": "Use Checks-Effects-Interactions",{C.RESET}'
                )
                print(
                    f'  {C.SYS}  │{C.RESET} {C.DIM} "code_example": "...ReentrancyGuard..."}}{C.RESET}'
                )
                print(
                    f"  {C.SYS}  └────────────────────────────────────────────────────────────────────┘{C.RESET}"
                )
                ...
                print(
                    f"  {C.HACK}│{C.RESET}  To fix {C.NEON_WHITE}SWC-107 Reentrancy{C.RESET}, use these patterns:"
                )
                print(f"  {C.HACK}│{C.RESET}")
                print(f"  {C.HACK}│{C.RESET}  {C.DATA}1. Checks-Effects-Interactions:{C.RESET}")
                print(f"  {C.HACK}│{C.RESET}     {C.DIM}// First: checks{C.RESET}")
                print(
                    f"  {C.HACK}│{C.RESET}     {C.HACK}require(balances[msg.sender] >= amount);{C.RESET}"
                )
                ...
                print(
                    f'  {C.HACK}│{C.RESET}     {C.HACK}(bool ok,) = msg.sender.call{{value: amount}}("");{C.RESET}'
                )
                print(f"  {C.HACK}│{C.RESET}")
                print(
                    f"  {C.HACK}│{C.RESET}  {C.DATA}2. Use OpenZeppelin ReentrancyGuard:{C.RESET}"
                )
                print(
                    f"  {C.HACK}│{C.RESET}     {C.HACK}function withdraw() external nonReentrant {{}}{C.RESET}"
                )
                print(
                    f"  {C.HACK}└──────────────────────────────────────────────────────────────────────┘{C.RESET}"
                )
                ...
                print(f"  {C.NEON_CYAN}│{C.RESET}")
                typing_effect(
                    '    "Generate a security report for this contract"',
                    delay=0.015,
                    color=C.NEON_WHITE,
                )
                print(f"  {C.NEON_CYAN}│{C.RESET}")
                ...
                print(
                    f'  {C.SYS}  │{C.RESET} {C.DIM}       "format": "markdown", "include_code": true}}{C.RESET}'
                )
                print(
                    f"  {C.SYS}  └────────────────────────────────────────────────────────────────────┘{C.RESET}"
                )
                ...
                print(
                    f"  {C.SYS}  │{C.RESET} {C.HACK}Status: 200 OK{C.RESET}  |  AI: Ollama (local)"
                )
                print(
                    f'  {C.SYS}  │{C.RESET} {C.DIM}{{"report_path": "output/VulnerableBank_audit.md",{C.RESET}'
                )
                print(
                    f'  {C.SYS}  │{C.RESET} {C.DIM} "sections": ["summary", "findings", "recommendations"],{C.RESET}'
                )
                ...
                print(f"  {C.HACK}│{C.RESET}")
                print(
                    f"  {C.HACK}│{C.RESET}  Report saved to: {C.DATA}output/VulnerableBank_audit.md{C.RESET}"
                )
                print(f"  {C.HACK}│{C.RESET}")
                print(f"  {C.HACK}│{C.RESET}  {C.DIM}Summary:{C.RESET}")
                print(
                    f"  {C.HACK}│{C.RESET}    • Total Findings: {C.NEON_WHITE}{len(findings)}{C.RESET}"
                )
                ...
                print(f"  {C.HACK}│{C.RESET}")
                print(
                    f"  {C.HACK}│{C.RESET}  {C.SYS}All analysis performed locally - code never left your machine{C.RESET}"
                )
                print(
                    f"  {C.HACK}└──────────────────────────────────────────────────────────────────────┘{C.RESET}"
                )
                ...
                print(
                    f"  {C.DATA}►{C.RESET} {C.NEON_WHITE}Natural Language:{C.RESET} No CLI commands needed - just ask questions"
                )
                print(
                    f"  {C.DATA}►{C.RESET} {C.NEON_WHITE}Data Sovereignty:{C.RESET} Code stays on YOUR machine (MCP is local)"
                )
                print(
                    f"  {C.DATA}►{C.RESET} {C.NEON_WHITE}AI Context:{C.RESET} Claude explains findings, not just raw output"
                )
                print(
                    f"  {C.DATA}►{C.RESET} {C.NEON_WHITE}25 Tools:{C.RESET} All accessible through simple conversation"
                )
                print(
                    f"  {C.DATA}►{C.RESET} {C.NEON_WHITE}$0 Cost:{C.RESET} Ollama + MCP = Free local AI security audits"
                )
...
Examples:
  ...
  python demo_thesis_defense.py --auto --speed 0.8   # Slower for reading
    ...
    demo = MIESCThesisDemo(
        auto_mode=args.auto, speed=args.speed, skip_slow=args.skip_slow, quick_demo=args.quick
    )
    demo.run()
