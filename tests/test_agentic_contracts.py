...
def test_erc4626_vault_inflation_plan_export_sanitizes_direct_instances():
    plan = ERC4626VaultInflationHardeningPlan(
        id="vault plan with spaces",
        objective="Harden empty-vault share conversion",
        surfaces=[
            VaultShareSurface(
                id="deposit surface",
                vault_function="deposit",
                share_function="convertToShares",
                total_assets_source="asset.balanceOf(address(this))",
                donation_exposure="direct donations move the exchange rate",
            )
        ],
        risks=[
            VaultInflationRisk(
                ...
                description="victim deposit can round to zero shares",
            )
        ],
        confidence=2.0,
    )
    ...
    assert exported["id"] == "vault plan with spaces"
    assert exported["surfaces"][0]["id"] == "deposit surface"
    assert exported["surfaces"][0]["vault_function"] == "deposit"
