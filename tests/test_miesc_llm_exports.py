...
def test_miesc_llm_erc20_token_compatibility_parser_is_usable():
    plans = public_llm.parse_erc20_token_compatibility_hardening_plans(
        {
            "erc20_token_compatibility_hardening_plans": [
                {
                    "objective": "deposit credits actual received token amount",
                    "surfaces": [
                        {
                            "id": "deposit_transfer_surface",
                            "entrypoint": "deposit",
                            "token_call": "safeTransferFrom",
                            "token_traits": ["fee_on_transfer"],
                        }
                    ],
                    "risks": [
                        {
                            ...
                            "description": "requested amount can exceed actual received",
                        }
                    ],
    ...
    assert len(plans) == 1
    assert isinstance(plans[0], public_llm.ERC20TokenCompatibilityHardeningPlan)
    assert plans[0].surfaces[0].entrypoint == "deposit"
    assert plans[0].surfaces[0].token_traits == ["fee_on_transfer"]
    assert plans[0].risks[0].category == "fee_on_transfer_misaccounting"
    assert plans[0].confidence == 0.82


def test_miesc_llm_account_abstraction_userop_parser_is_usable():
    plans = public_llm.parse_account_abstraction_userop_hardening_plans(
        {
            "account_abstraction_userop_hardening_plans": [
                {
                    "objective": "validateUserOp is EntryPoint gated",
                    "surfaces": [
                        {
                            "id": "validate_userop_surface",
                            "component": "account",
                            "entrypoint_function": "validateUserOp",
                            "userop_fields": ["sender", "nonce", "signature"],
                        }
                    ],
                    "risks": [
                        {
                            ...
                            "description": "validation can be called outside EntryPoint",
                        }
                    ],
                    "validation_tests": ["non-EntryPoint validateUserOp call reverts"],
                    "confidence": 0.79,
                }
            ]
        }
    ...
    assert len(plans) == 1
    assert isinstance(plans[0], public_llm.AccountAbstractionUserOpHardeningPlan)
    assert plans[0].surfaces[0].component == "account"
    assert plans[0].surfaces[0].entrypoint_function == "validateUserOp"
    assert plans[0].risks[0].category == "entrypoint_guard"
    assert plans[0].confidence == 0.79


def test_miesc_llm_external_call_returndata_parser_is_usable():
    plans = public_llm.parse_external_call_returndata_hardening_plans(
        {
            "external_call_returndata_hardening_plans": [
                {
                    "objective": "staticcall return data is bounded before decode",
                    "surfaces": [
                        {
                    ...
                    "risks": [
                        {
                            ...
                            "description": "returned bytes are decoded without length bound",
                        }
                    ],
    ...
    assert len(plans) == 1
    assert isinstance(plans[0], public_llm.ExternalCallReturndataHardeningPlan)
...
def test_miesc_llm_financial_math_precision_parser_is_usable():
    plans = public_llm.parse_financial_math_precision_hardening_plans(
        {
            "financial_math_precision_hardening_plans": [
                {
                    "objective": "reward math preserves fixed point precision",
                    "surfaces": [
                        {
                    ...
                    "risks": [
                        {
                            ...
                            "description": "division truncates before fixed-point scaling",
                        }
                    ],
                    "validation_tests": ["tiny reward values do not round to zero"],
                    "confidence": 0.8,
                }
            ]
        }
    ...
    assert len(plans) == 1
    assert isinstance(plans[0], public_llm.FinancialMathPrecisionHardeningPlan)
    assert plans[0].surfaces[0].entrypoint == "accrue"
    assert plans[0].surfaces[0].operation_order == "division_before_multiplication"
