...
class TestAuditPipelineCLI:
    ...
    def test_audit_quick_ci_mode(self, cli_runner, vulnerable_contract):
        ...
        with patch('miesc.cli.main._run_tool') as mock_run:
            ...
            result = cli_runner.invoke(
                cli, ['audit', 'quick', vulnerable_contract, '--ci']
            )
    ...
    def test_audit_smart_with_mock_tools(self, cli_runner, vulnerable_contract):
        ...
        with patch('miesc.cli.main._run_tool') as mock_run, \
            ...
            result = cli_runner.invoke(
                cli, ['audit', 'smart', vulnerable_contract]
            )
    ...
    def test_audit_with_nonexistent_file(self, cli_runner):
        ...
        result = cli_runner.invoke(
            cli, ['audit', 'quick', '/tmp/nonexistent_contract_12345.sol']
        )
...
class TestResultAggregation:
    ...
    def test_severity_normalization(self):
        ...
        findings = [
            {'type': 'test', 'severity': 'CRITICAL', 'message': 'crit', 'location': {'file': 'a.sol', 'line': 1}},
            {'type': 'test2', 'severity': 'High', 'message': 'high', 'location': {'file': 'a.sol', 'line': 5}},
            {'type': 'test3', 'severity': 'med', 'message': 'med', 'location': {'file': 'a.sol', 'line': 10}},
            {'type': 'test4', 'severity': 'Info', 'message': 'info', 'location': {'file': 'a.sol', 'line': 15}},
            {'type': 'test5', 'severity': 'note', 'message': 'note', 'location': {'file': 'a.sol', 'line': 20}},
        ]
        ...
        valid_severities = {
            ...
            'informational', 'info', 'note', 'warning', 'optimization',
        }
...
class TestCorrelationEngineIntegration:
    ...
    def test_correlate_overlapping_reentrancy_findings(
        ...
        reentrancy = [
            f for f in correlated if f.canonical_type == 'reentrancy'
        ]
        ...
        for finding in correlated:
            ...
            if finding.is_cross_validated:
                single_tool = [
                    ...
                    if not f.is_cross_validated
                ]
                if single_tool:
                    avg_cv_fp = finding.false_positive_probability
                    avg_single_fp = sum(
                        f.false_positive_probability for f in single_tool
                    ) / len(single_tool)
...
class TestMLPipelineEndToEnd:
    ...
    def test_ml_pipeline_preserves_critical_findings(self):
        ...
        critical_types = [
            f.get('type') for f in result.filtered_findings
