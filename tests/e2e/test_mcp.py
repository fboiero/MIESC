...
def test_context_bus_messaging():
    ...
    try:
        ...
        assert len(bus.message_history) == 1, \
            f"Message Publishing failed: Expected 1 message, got {len(bus.message_history)}"
        logger.info(f"✅ PASS: Message Publishing - {len(bus.message_history)} message")
...
def test_audit_trail_export():
    ...
    try:
        ...
        assert audit_trail_path.exists(), \
            f"Audit Trail Export failed: Audit trail should be exported to {audit_trail_path}"
        logger.info(f"✅ PASS: Audit Trail Export - exported to {audit_trail_path}")

        # Verify JSON structure
        with open(audit_trail_path, 'r') as f:
            audit_data = json.load(f)

        assert "protocol" in audit_data and "messages" in audit_data, \
            "Audit Trail Structure failed: Audit trail should have protocol and messages"
        logger.info("✅ PASS: Audit Trail Structure - protocol and messages present")

        assert len(audit_data["messages"]) > 0, \
            f"Audit Trail Messages failed: Expected messages in audit trail, got {len(audit_data['messages'])}"
        logger.info(f"✅ PASS: Audit Trail Messages - {len(audit_data['messages'])} messages")

    except Exception as e:
        pytest.fail(f"Audit Trail Export failed: {str(e)}")
