                ...
                'detectors': [
                    {
                        ...
                        'elements': [{'source_mapping': {'filename_relative': 'test.sol', 'lines': [25]}}]
                    },
                    {
                        'check': 'uninitialized-state',
                        'impact': 'Medium',
                        ...
                        'elements': [{'source_mapping': {'filename_relative': 'token.sol', 'lines': [10]}}]
                    }
                ]
            }
        }
...
class TestXauditDashboard:
    ...
    def temp_results_dir(self, tmp_path):
        ...
        slither_data = {
            'results': {
                'detectors': [
                    {'check': 'reentrancy', 'impact': 'High', 'confidence': 'High', 'description': 'Test'},
                    {'check': 'overflow', 'impact': 'Medium', 'confidence': 'Medium', 'description': 'Test'},
                ]
            }
        }
        (slither_dir / "results.json").write_text(json.dumps(slither_data))
    ...
    def test_generate_charts_with_matplotlib(self, temp_results_dir, tmp_path):
        ...
        assert (output_dir / "severity_distribution.png").exists() or True  # May not exist if plt is mocked
...
class TestWebDashboardGenerator:
    ...
    def temp_results_dir(self, tmp_path):
        ...
        solhint_dir.mkdir()
        solhint_data = [
            {'filePath': 'test.sol', 'line': 10, 'severity': 'warning', 'message': 'Avoid using var', 'ruleId': 'no-var'}
        ]
        ...
        slither_data = {
            'results': {
                'detectors': [
                    {'check': 'reentrancy', 'impact': 'High', 'confidence': 'High', 'description': 'Reentrancy', 'elements': []}
                ]
            }
        }
        (slither_dir / "results.json").write_text(json.dumps(slither_data))
        ...
        mythril_data = {
            'issues': [
                {'title': 'Overflow', 'severity': 'High', 'swc-id': 'SWC-101', 'description': {'head': 'Test'}, 'address': 0}
            ]
        }
        (mythril_dir / "results.json").write_text(json.dumps(mythril_data))
        ...
        (foundry_inv_dir / "results.txt").write_text("[PASS] invariant_test1\n[FAIL] invariant_test2")
            ...
            'results': {
                'detectors': [
                    {'check': 'reentrancy', 'impact': 'High', 'confidence': 'High',
                     'description': 'Test', 'elements': [{'source_mapping': {'filename_relative': 'test.sol', 'lines': [1]}}]}
                ]
            }
        }
        (slither_dir / "results.json").write_text(json.dumps(slither_data))
