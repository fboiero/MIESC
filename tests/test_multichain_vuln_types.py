...
class TestCrossChainConsistency:
    ...
    @pytest.mark.parametrize("AnalyzerClass", [
        StellarAnalyzer, SolanaAnalyzer, MoveAnalyzer,
    ])
    ...
    def test_total_vuln_types_across_chains(self):
        total = (
            len(StellarVulnerability)
            + len(SolanaVulnerability)
            + len(MoveVulnerability)
        )
        assert total >= 60, (
