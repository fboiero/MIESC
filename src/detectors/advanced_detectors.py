...
class RugPullDetector:
    ...
    HIDDEN_MINT_PATTERNS = [
        ...
        (r"totalSupply\s*\+?=", "Direct totalSupply modification - bypasses mint controls"),
    ]
    ...
    def detect(self, source_code: str, file_path: Optional[Path] = None) -> List[AdvancedFinding]:
        ...
        all_patterns = (
            self.OWNER_DRAIN_PATTERNS + self.HIDDEN_MINT_PATTERNS + self.BLACKLIST_PATTERNS
        )
        ...
        for i, line in enumerate(lines, 1):
            for pattern, description in all_patterns:
                if re.search(pattern, line, re.IGNORECASE):
                    severity = (
                        Severity.CRITICAL if "drain" in description.lower() else Severity.HIGH
                    )
                    findings.append(
                        AdvancedFinding(
...
class GovernanceDetector:
    ...
    def detect(self, source_code: str, file_path: Optional[Path] = None) -> List[AdvancedFinding]:
        ...
        is_governance = re.search(
            r"Governor|Voting|Proposal|DAO|governance", source_code, re.IGNORECASE
        )

        if not is_governance:
            return findings

        all_patterns = self.FLASH_LOAN_VOTE_PATTERNS + self.TIMELOCK_PATTERNS + self.QUORUM_PATTERNS
...
class TokenSecurityDetector:
    ...
    # Hidden fee patterns
    HIDDEN_FEE_PATTERNS = [
        (r"(?:tax|fee|burn)[Pp]ercent\s*=\s*\d{2,}", "High transfer fee percentage detected"),
        (
            r"function\s+\w*[Ss]et(?:Tax|Fee)\w*\s*\([^)]*\)\s*(?:external|public)",
            "Fee can be changed after deployment - fee manipulation risk",
        ),
        (r"(?:buy|sell)[Ff]ee\s*>\s*(?:sell|buy)[Ff]ee", "Asymmetric buy/sell fees detected"),
    ]
    ...
    MAX_TX_PATTERNS = [
        ...
        (r"require\s*\([^)]*<\s*max[Tt]x", "Transaction size limited - check if owner can modify"),
        (
                ...
                if re.search(pattern, line, re.IGNORECASE):
                    ...
                    findings.append(
                        AdvancedFinding(
                            ...
                            references=["https://tokensniffer.com/", "https://honeypot.is/"],
                        )
                    )
...
class ProxyUpgradeDetector:
    ...
    STORAGE_PATTERNS = [
        (r"assembly\s*{[^}]*sstore", "Direct storage manipulation - potential storage collision"),
        (
    ...
    DELEGATECALL_PATTERNS = [
        ...
        (r"\.delegatecall\s*\(\s*[^)]*\)", "Delegatecall detected - ensure target is trusted"),
    ]
        ...
        for i, line in enumerate(lines, 1):
            for pattern, description in all_patterns:
                if re.search(pattern, line, re.IGNORECASE):
                    ...
                    severity = (
                        Severity.CRITICAL if "upgrade" in description.lower() else Severity.HIGH
                    )
...
class CentralizationDetector:
    ...
    CENTRALIZATION_PATTERNS = [
        (r"onlyOwner[^}]*(?:selfdestruct|suicide)", "Owner can destroy contract - total loss risk"),
        (r"onlyOwner[^}]*(?:pause|unpause)", "Owner can pause contract - centralization risk"),
        (
...
def main():
    ...
    print("\n" + "=" * 60)  # noqa: T201
    print("  MIESC Advanced Vulnerability Scanner")  # noqa: T201
    print("=" * 60 + "\n")  # noqa: T201
