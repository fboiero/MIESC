    ...
    def _run_git_command(self, cmd: List[str], project_root: str) -> str:
        ...
        try:
            result = subprocess.run(
                ["git"] + cmd, cwd=project_root, capture_output=True, text=True, timeout=30
            )
    ...
    def analyze_code_maturity(self, project_root: str) -> Dict[str, Any]:
        ...
        try:
            ...
            first_commit_date = self._run_git_command(
                ["log", "--reverse", "--format=%ct", "--max-parents=0"], project_root
            )
            ...
            # 2. Total commits
            total_commits_str = self._run_git_command(["rev-list", "--count", "HEAD"], project_root)
            total_commits = int(total_commits_str) if total_commits_str else 0

            # 3. Contributors
            contributors_output = self._run_git_command(["shortlog", "-sn", "--all"], project_root)
            ...
            recent_commits_str = self._run_git_command(
                ["rev-list", "--count", "--since=90.days.ago", "HEAD"], project_root
            )
            commits_last_90_days = int(recent_commits_str) if recent_commits_str else 0

            # 5. Days since last commit
            last_commit_date = self._run_git_command(["log", "-1", "--format=%ct"], project_root)
            ...
            maturity_factors = {
                ...
                "recent_activity": min(
                    commits_last_90_days / 10, 1.0
                ),  # 10+ commits in 90 days = active
            ...
            logger.info(
                f"Age: {age_days:.0f} days, Commits: {total_commits}, Contributors: {contributors}"
            )
            ...
            return {"error": f"Git parsing error: {e}", "passes_threshold": False}
        except Exception as e:
    ...
    def analyze_commit_patterns(self, project_root: str) -> Dict[str, Any]:
        ...
        try:
            ...
            commits_output = self._run_git_command(
                ["log", "--oneline", "--shortstat", "--since=6.months.ago"], project_root
            )

            if not commits_output:
                return {"error": "No recent commits found", "stability_score": 0.0}

            # Count commits
            commit_lines = [
                ln for ln in commits_output.split("\n") if ln.strip() and "changed" not in ln
            ]
            ...
            large_commits = 0
            stat_lines = [ln for ln in commits_output.split("\n") if "changed" in ln]
