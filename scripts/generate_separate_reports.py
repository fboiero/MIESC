...
def create_spanish_report():
    ...
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run(
        "Documentación del Proceso de Desarrollo, Decisiones de Diseño\ny Resultados Experimentales Detallados"
    )
    run.font.size = Pt(11)
    ...
    p = doc.add_paragraph()
    p.add_run(
        """MIESC (Multi-layer Intelligent Evaluation for Smart Contracts) representa un avance significativo en el campo de la seguridad de contratos inteligentes. Este framework integrado combina 25 herramientas de análisis distribuidas en 7 capas de defensa, siguiendo el principio de Defense-in-Depth ampliamente reconocido en ciberseguridad.
...
El presente documento detalla exhaustivamente el proceso de desarrollo de MIESC desde su concepción hasta la versión 4.0.0, incluyendo todas las decisiones arquitectónicas, los desafíos técnicos superados, y los resultados experimentales obtenidos mediante una metodología científica rigurosa basada en Wohlin et al. (2012)."""
    ...
    p = doc.add_paragraph()
    p.add_run(
        """La tecnología blockchain ha experimentado un crecimiento exponencial, con Ethereum emergiendo como la plataforma líder para aplicaciones descentralizadas (dApps). Los contratos inteligentes gestionan actualmente miles de millones de dólares en activos digitales.
...
• Euler Finance (2023): $197 millones comprometidos"""
    ...
    p = doc.add_paragraph()
    p.add_run(
        """La arquitectura de MIESC se basa en el principio de Defense-in-Depth, ampliamente utilizado en ciberseguridad. Este principio establece que múltiples capas de control proporcionan mayor protección que una única capa robusta.
...
Cada capa de MIESC aborda diferentes aspectos del análisis de seguridad, de modo que una vulnerabilidad que escape a una capa tiene alta probabilidad de ser detectada por otra. Las 7 capas se ejecutan en paralelo cuando es posible, maximizando la eficiencia del análisis."""
    ...
    doc.add_paragraph()
    add_figure_with_image(
        doc, "architecture_7_layers.svg", "Arquitectura de 7 Capas de MIESC", "4.1"
    )
    ...
    p = doc.add_paragraph()
    p.add_run(
        """El patrón Adapter es el mecanismo principal de integración de herramientas. Cada herramienta se encapsula en un adapter que traduce su interfaz específica a la interfaz común de MIESC, siguiendo los principios SOLID:
...
• Dependency Inversion: Dependencia de abstracciones, no implementaciones"""
    ...
    doc.add_paragraph()
    add_figure_with_image(doc, "adapter_pattern.svg", "Implementación del Patrón Adapter", "4.2")
...
8. Salida: Reporte unificado (JSON, SARIF, PDF, Markdown)""")
    ...
    p = doc.add_paragraph()
    p.add_run(
        """La validación de MIESC siguió una metodología experimental rigurosa basada en las guías de Wohlin et al. (2012) para experimentación en ingeniería de software.
...
• H4: El overhead es menor al 50% del tiempo secuencial"""
    ...
    add_figure_with_image(
        doc, "results_comparison.svg", "Comparación de Resultados Experimentales", "6.1"
    )
    ...
    p = doc.add_paragraph()
    p.add_run(
        """Las herramientas basadas en Machine Learning e Inteligencia Artificial contribuyen significativamente a la detección de vulnerabilidades que los métodos tradicionales no detectan."""
    )
    ...
    p = doc.add_paragraph()
    p.add_run(
        """El 17% de detecciones únicas de las herramientas ML/AI supera la hipótesis H3 (≥15%), confirmando el valor agregado de la integración de inteligencia artificial en el análisis de seguridad de contratos inteligentes."""
    )
    ...
    hypothesis_results = [
        ...
        (
            "H3: ML/AI Detecciones",
            "≥15% detecciones únicas",
            "17.0% únicas",
            "CONFIRMADA",
            "p<0.01",
        ),
    ...
    p = doc.add_paragraph()
    p.add_run(
        """Todas las hipótesis planteadas han sido confirmadas con significancia estadística (p<0.05 en todos los casos). Los resultados demuestran que MIESC logra sus objetivos de:
...
4. Mantener tiempos de análisis competitivos mediante paralelización"""
    ...
    p = doc.add_paragraph()
    p.add_run(
        """Este trabajo ha presentado MIESC, un framework integrado de análisis de seguridad para contratos inteligentes que combina 25 herramientas en una arquitectura de 7 capas basada en el principio de Defense-in-Depth.
...
def create_english_report():
    ...
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run(
        "Development Process Documentation, Design Decisions\nand Detailed Experimental Results"
    )
    run.font.size = Pt(11)
...
This document exhaustively details MIESC's development process from its conception to version 4.0.0, including all architectural decisions, technical challenges overcome, and experimental results obtained through a rigorous scientific methodology based on Wohlin et al. (2012)."""
    ...
    p = doc.add_paragraph()
    p.add_run(
        """Blockchain technology has experienced exponential growth, with Ethereum emerging as the leading platform for decentralized applications (dApps). Smart contracts currently manage billions of dollars in digital assets.
...
• Euler Finance (2023): $197 million compromised"""
    ...
    p = doc.add_paragraph()
    p.add_run(
        """MIESC's architecture is based on the Defense-in-Depth principle, widely used in cybersecurity. This principle establishes that multiple layers of control provide greater protection than a single robust layer.
...
Each MIESC layer addresses different aspects of security analysis, so that a vulnerability that escapes one layer has a high probability of being detected by another. The 7 layers run in parallel when possible, maximizing analysis efficiency."""
    ...
    doc.add_paragraph()
    add_figure_with_image(doc, "architecture_7_layers.svg", "MIESC 7-Layer Architecture", "4.1")
    ...
    p = doc.add_paragraph()
    p.add_run(
        """The Adapter pattern is the main mechanism for tool integration. Each tool is encapsulated in an adapter that translates its specific interface to MIESC's common interface, following SOLID principles:
...
• Dependency Inversion: Dependency on abstractions, not implementations"""
    ...
    doc.add_paragraph()
    add_figure_with_image(doc, "adapter_pattern.svg", "Adapter Pattern Implementation", "4.2")
...
8. Output: Unified report (JSON, SARIF, PDF, Markdown)""")
    ...
    p = doc.add_paragraph()
    p.add_run(
        """MIESC validation followed a rigorous experimental methodology based on Wohlin et al. (2012) guidelines for software engineering experimentation.
...
• H4: Overhead is less than 50% of sequential time"""
    ...
    add_figure_with_image(doc, "results_comparison.svg", "Experimental Results Comparison", "6.1")
    ...
    p = doc.add_paragraph()
    p.add_run(
        """Machine Learning and Artificial Intelligence based tools contribute significantly to detecting vulnerabilities that traditional methods miss."""
    )
    ...
    p = doc.add_paragraph()
    p.add_run(
        """The 17% unique detections from ML/AI tools exceeds hypothesis H3 (≥15%), confirming the added value of artificial intelligence integration in smart contract security analysis."""
    )
    ...
    p = doc.add_paragraph()
    p.add_run(
        """All hypotheses have been confirmed with statistical significance (p<0.05 in all cases). Results demonstrate that MIESC achieves its objectives of:
...
4. Maintaining competitive analysis times through parallelization"""
    ...
    p = doc.add_paragraph()
    p.add_run(
        """This work has presented MIESC, an integrated security analysis framework for smart contracts that combines 25 tools in a 7-layer architecture based on the Defense-in-Depth principle.
