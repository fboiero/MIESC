...
def print_header():
    ...
    print("=" * 70)
    print("MIESC Adapter Diagnostic Tool v5.4.2")
...
def init_adapters():
    ...
    try:
        ...
        print(
            f"Registered {result.get('registered', 0)}/{result.get('total_adapters', 0)} adapters"
        )
        print()
        return result
...
def diagnose_all_adapters():
    ...
    adapter_statuses = status_report.get("adapters") or status_report.get("tools", [])

    # Organize by layer
    by_layer = {}
    for adapter_info in adapter_statuses:
        layer = adapter_info.get("layer") or adapter_info.get("category", "unknown")
    ...
    for layer_key, (layer_num, layer_name) in LAYER_MAP.items():
        ...
        for adapter in sorted(adapters, key=lambda x: x.get("name", "")):
            ...
            try:
                status = (
                    ToolStatus[status_str.upper()] if isinstance(status_str, str) else status_str
                )
    ...
    # Handle categories not present in LAYER_MAP
    unknown_adapters = []
    ...
    if unknown_adapters or "unknown" in by_layer:
        ...
        for adapter in all_unknown:
            ...
            if status == ToolStatus.AVAILABLE:
                ...
                failed += 1
                failed_list.append((name, category, error))
            elif status == ToolStatus.CONFIGURATION_ERROR:
                config_error += 1
                config_list.append((name, category, error))
            elif status == ToolStatus.LICENSE_REQUIRED:
                license_required += 1
                license_list.append((name, category, error))
            elif status == ToolStatus.DEPRECATED:
                deprecated += 1
            ...
            version_str = f" - {version}" if version else ""
            error_str = f" ({error})" if error and status != ToolStatus.AVAILABLE else ""
            print(f"{color}{label}{Colors.NC} {name}{version_str} [{category}]{error_str}")
        print()
    ...
    report = {
        ...
        "failed": [{"name": n, "layer": layer, "error": e} for n, layer, e in failed_list],
        "config_errors": [{"name": n, "layer": layer, "error": e} for n, layer, e in config_list],
        "license_required": [
            {"name": n, "layer": layer, "error": e} for n, layer, e in license_list
        ],
