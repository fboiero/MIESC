...
COMPOSE_DEV_FILE="$PROJECT_DIR/docker/docker-compose.yml"
COMPOSE_PROD_FILE="$PROJECT_DIR/docker/docker-compose.prod-llm.yml"
...
select_mode() {
    ...
    echo ""
    echo "  1) Development (docker/docker-compose.yml)"
    echo "     - Basic setup, models loaded on demand"
    echo "     - Good for testing and development"
    echo ""
    echo "  2) Production (docker/docker-compose.prod-llm.yml)"
...
start_services() {
    ...
    if [ "$DEPLOYMENT_MODE" = "production" ]; then
        ...
        echo "This will download ~8GB of LLM models on first run."
        echo ""
        $COMPOSE_CMD -f "$COMPOSE_FILE" --profile llm up -d --build ollama ollama-init
    fi
...
run_health_check() {
    ...
    HEALTH_SCRIPT="$PROJECT_DIR/docker/health-check.sh"
...
print_usage() {
    ...
    echo "  $COMPOSE_CMD -f docker/docker-compose.yml --profile dev run --rm miesc-shell miesc audit quick /app/contracts/VulnerableBank.sol -o /data/results.json"
    echo ""
    echo "  # Generate premium report with LLM interpretation"
    echo "  $COMPOSE_CMD -f docker/docker-compose.yml --profile dev run --rm miesc-shell miesc report /data/results.json -t premium --llm-interpret -o /data/report.md"
    echo ""
    echo "  # Interactive shell"
    if [ "$DEPLOYMENT_MODE" = "production" ]; then
        echo "  docker exec -it miesc-prod /bin/bash"
    else
        echo "  $COMPOSE_CMD -f docker/docker-compose.yml --profile dev run miesc-shell"
    fi
    echo ""
    ...
    echo "  # View logs"
    if [ "$DEPLOYMENT_MODE" = "production" ]; then
        echo "  $COMPOSE_CMD -f docker/docker-compose.prod-llm.yml logs -f ollama"
    else
        echo "  $COMPOSE_CMD -f docker/docker-compose.yml logs -f ollama"
    fi
    echo ""
    echo "  # Stop services"
    if [ "$DEPLOYMENT_MODE" = "production" ]; then
        echo "  $COMPOSE_CMD -f docker/docker-compose.prod-llm.yml down"
    else
        echo "  $COMPOSE_CMD -f docker/docker-compose.yml --profile llm down"
    fi
    echo ""
