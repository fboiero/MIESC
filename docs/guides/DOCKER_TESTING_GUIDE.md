# Testing MIESC Docker Images v6.0.0
...
Both commands should show **MIESC version 6.0.0**.
...
| Tag | Contents |
|-----|----------|
| `miesc:latest` / `miesc:6.0.0` | Standard: Slither, Aderyn, Solhint, Foundry (~15 tools) |
| `miesc:full` / `miesc:6.0.0-full` | Full: + Mythril, Manticore, Echidna, Halmos, PyTorch (~30 tools) |

> **Note:** Ignore any older versions (pre-6.0.0) that may appear in the registry. Always use the tags listed above.
