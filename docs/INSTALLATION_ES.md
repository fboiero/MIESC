# MIESC v5.0.3 - Guía de Instalación Completa
...
```bash
docker build -t miesc:v5.0.3 .
docker run --rm -v $(pwd):/contracts miesc:v5.0.3 audit quick /contracts/MiContrato.sol
```

### x86_64 (Intel/AMD)

```bash
docker build --platform linux/amd64 -f Dockerfile.x86 -t miesc:v5.0.3-x86 .
docker run --platform linux/amd64 --rm -v $(pwd):/contracts miesc:v5.0.3-x86 audit quick /contracts/MiContrato.sol
```
...
```
MIESC v5.0.3 - Verificación de Instalación
============================================================
...
**Versión**: 5.0.3 | **Última Actualización**: Febrero 2026
