...
MIESC supports multiple analysis modes through a unified pipeline: the same audit prompt, RAG enrichment, and finding normalization is applied regardless of provider. This enables fair comparison across six configurations (Table~\ref{tab:evmbench}).

\begin{table}[htbp]
\caption{EVMBench Recall by Provider (10 audits, 17 vulnerabilities)}
\label{tab:evmbench}
\centering
\begin{tabular}{@{}lcccrr@{}}
\toprule
\textbf{Mode} & \textbf{Recall} & \textbf{Prec.} & \textbf{F1} & \textbf{Cost} & \textbf{Local} \\
\midrule
Static only & 11.8\% & --- & --- & \$0 & \checkmark \\
+ Ollama (interpret only) & 17.6\% & 13.0\% & 15.0\% & \$0 & \checkmark \\
+ Ollama qwen2.5-coder:32b & 82.4\% & 20.0\% & 32.2\% & \$0 & \checkmark \\
+ OpenAI GPT-4o & 64.7\% & 22.4\% & 33.3\% & \$0.30 & \\
+ Claude Sonnet 4.6 & 94.1\% & 25.0\% & 39.5\% & \$1.35 & \\
+ Claude Opus 4.6 & 88.2\% & 23.8\% & 37.5\% & \$4 & \\
\midrule
\multicolumn{6}{@{}l}{\textit{Full-scale (40 audits, 120 vulns):}} \\
+ GPT-4o & 73.7\% & 20.0\% & 31.5\% & \$1.20 & \\
+ Claude Sonnet 4.6 & 82.5\% & 34.0\% & 48.2\% & \$5.50 & \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:evmbench} reveals four key findings. First, static analysis alone achieves only 11.8\% recall on business logic vulnerabilities, confirming that pattern-based tools cannot detect semantic flaws. Second, a local 32-billion-parameter model (qwen2.5-coder:32b via Ollama) achieves 82.4\% recall with \emph{zero API cost and full data locality}---surpassing GPT-4o (64.7\%) and approaching Claude Opus (88.2\%). Third, Claude Sonnet 4.6 achieves 94.1\% recall at \$1.35 per audit, outperforming the larger Opus model (88.2\% at \$4)---suggesting that model scale is less important than prompt engineering and tool orchestration for security auditing. Fourth, all MIESC configurations substantially outperform Claude Opus as a standalone auditor (45.9\% on the EVMBench leaderboard), demonstrating that \textbf{orchestrated static analysis combined with any LLM is strictly superior to even the best LLM alone}.

The practical implication is significant: a security researcher can run MIESC with a local model at zero cost and achieve recall competitive with commercial AI audit services, while adding a frontier API (\$1.35/audit vs.\ \$20K--60K for manual audits) pushes recall to 94\%.
...
The central finding of this work is that no single analysis technique suffices for smart contract security, but \emph{any} LLM combined with static analysis orchestration outperforms even the best LLM alone. On pattern-based vulnerabilities (SmartBugs), orchestrating multiple static tools raises recall from 43\% (Slither alone) to 80\%. On business-logic vulnerabilities (EVMBench), static analysis alone achieves only 11.8\%---but combining it with a local 32B-parameter model reaches 82.4\% at zero cost, and 94.1\% with Claude Sonnet at \$1.35 per audit. Both configurations substantially outperform Claude Opus as a standalone auditor (45.9\% on the leaderboard). This demonstrates that \textbf{orchestrated static analysis + LLM is strictly superior to LLM alone}, and that the orchestration matters more than the model: a free local model outperforms GPT-4o API (82.4\% vs.\ 64.7\%).

MIESC's practical contribution is making this accessible: \texttt{miesc scan contract.sol -{}-model ollama} runs Slither, Aderyn, an intelligence engine with Bayesian cross-tool confidence, protocol-specific RAG enrichment, and a local LLM audit---entirely free, offline, and DPGA-compliant. Adding a frontier API (\texttt{-{}-model claude}, \$1.35/audit) pushes recall to 94\%, compared with \$20K--60K for a manual audit engagement.
