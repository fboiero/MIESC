...
\author{
\IEEEauthorblockN{Fernando Boiero, Sebastian Norberto Mussetta, and Norberto Gaspar Cena}
\IEEEauthorblockA{
...
Table~\ref{tab:smartbugs} shows the latest full-corpus reproducible SmartBugs profile stored in the repository as \texttt{paper1\_smartbugs\_eval\_layers\_1\_6\_7.json}. MIESC reaches 95.8\% recall and 35.9\% F1, substantially above the single-tool baselines reported by Durieux et al.~\cite{durieux2020empirical}. This benchmark is useful for classical vulnerability patterns, but it is not the central evidence for business-logic vulnerabilities; that role is played by EVMBench (Section~\ref{sec:evmbench}).
...
The results show a clear split between classical pattern detection and semantic vulnerability detection. On SmartBugs-curated, MIESC's static+intelligence profile (layers 1, 6, 7 plus the pattern-based intelligence engine) reaches 95.8\% recall (137/143) without invoking any LLM. The 6 remaining misses---3 ``other'' (heterogeneous, undefined vulnerability class), 2 front-running (transaction-ordering semantics), and 1 short-addresses (a deprecated attack vector)---require reasoning about protocol-level invariants that regex patterns cannot express. When a local 32B model (qwen2.5-coder:32b via Ollama) is added for only those missed contracts, it recovers the 2 front-running and the short-addresses contracts, raising recall to \textbf{140/143 (97.9\%)} at zero API cost; we report this lift as a secondary follow-up backed by the v2 SmartBugs local-Ollama artifact. The 3 residual misses are ``other''-category contracts with no well-defined vulnerability class in the SmartBugs ground truth.

On EVMBench, static-only analysis reaches 18.3\% recall, while LLM providers detect complementary subsets of business-logic vulnerabilities and the four-provider union closes 12 otherwise missed findings beyond the best single provider (Claude Sonnet 4.6, 99/120). The key contribution is therefore not merely adding more tools, but using each layer for the vulnerability class it is suited to: static tools for known patterns, specialized detectors for domain-specific signals, and multi-provider LLMs for semantic reasoning.
