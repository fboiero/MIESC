...
Table~\ref{tab:smartbugs} shows the latest full-corpus reproducible SmartBugs profile stored in the repository as \texttt{paper1\_vnext\_clean\_20260621.json}. MIESC reaches 99.3\% recall and 41.8\% F1 with layer-1 static analysis plus the pattern-based intelligence engine (no LLM), substantially above the single-tool baselines reported by Durieux et al.~\cite{durieux2020empirical}. This benchmark is useful for classical vulnerability patterns, but it is not the central evidence for business-logic vulnerabilities; that role is played by EVMBench (Section~\ref{sec:evmbench}).
...
Unchecked Low-Level Calls & 45.2\% & \textbf{100\%} & 62.3\% \\
...
\vspace{2pt}
\footnotesize{Latest full-corpus local JSON: \texttt{paper1\_vnext\_clean\_20260621.json}; layer 1 + intelligence engine; 143 contracts. Tool-level failures on legacy syntax are counted as per-contract misses when no other layer detects the category.}
\end{table}

Eight of ten categories achieve \textbf{100\% recall}: reentrancy, unchecked low-level calls, arithmetic, time manipulation, access control, bad randomness, denial of service, and front running (partial). The weakest categories are short addresses (a deprecated attack vector) and the heterogeneous ``other'' class (3 contracts with no clear vulnerability pattern). Front running achieves only 25\% recall because 3 of 4 contracts require semantic reasoning about transaction ordering that static patterns cannot capture. These residual weaknesses motivate the EVMBench analysis, where semantic and protocol-level reasoning are measured directly.
...
Table~\ref{tab:performance} summarizes MIESC's execution speed for the reproducible SmartBugs full-corpus profile. The run completed all 143 contracts in 484 seconds, or 17.7 contracts per minute.
...
Total execution time & 484 seconds \\
Average time per contract & 3.4 seconds \\
Throughput & 17.7 contracts/minute \\
...
The results show a clear split between classical pattern detection and semantic vulnerability detection. On SmartBugs-curated, MIESC's layer-1 static analysis plus the pattern-based intelligence engine reaches \textbf{99.3\% recall (142/143)} without invoking any LLM, exceeding the prior multi-layer (1,6,7) profile while using fewer layers. The 6 misses reported in earlier versions---3 ``other'', 2 front-running, and 1 modifier-based reentrancy---are now closed by deterministic pattern-engine detectors added this cycle: an uninitialized storage-pointer detector (SWC-109) for the ``other'' cases, relaxed front-running ordering patterns, and a caller-callback reentrancy pattern. The sole remaining miss is a short-address case whose signature is structurally indistinguishable from ordinary token transfers (Section~\ref{sec:missed}); see the multi-layer ablation below for why adding the ML layer to close it is not worthwhile.

On EVMBench, static-only analysis reaches 18.3\% recall, while LLM providers detect complementary subsets of business-logic vulnerabilities and the four-provider union closes 18 otherwise missed findings beyond the best single provider. The key contribution is therefore not merely adding more tools, but using each layer for the vulnerability class it is suited to: static tools for known patterns, specialized detectors for domain-specific signals, and multi-provider LLMs for semantic reasoning.

\subsubsection{Multi-Layer Recall Ablation}
Because layer-1 plus the intelligence engine already reaches 100\% recall on 9 of the 10 SmartBugs categories, the only reachable additional gain is the single short-address false negative. Adding the ML layer (layer~6) closes it (143/143) but at a cost of 42 additional false positives across the other 142 contracts---its detector flags essentially every \texttt{(address, uint)} token function---while the specialized layer (layer~7) adds no recall. We therefore report the layer-1 + intelligence profile as the recall-safe configuration: it maximizes recall without the precision collapse the ML short-address detector introduces.
...
\textbf{Tool stability.} Two external static analyzers crash on certain legacy Solidity AST shapes (\texttt{aderyn} panics on \texttt{Throw}/\texttt{BlockOrStatement}; \texttt{slither} raises IR-generation assertions on a few contracts). On an unloaded machine the aggregate recall is a stable 99.3\%; under heavy concurrent CPU load the crash rate rises and recall can dip to 97.2\% when a crashed tool's findings are not also covered by the intelligence engine. The intelligence-engine detectors are deterministic and load-independent. We report the clean-machine number, the harness logs per-run crash counts, and we recommend running the benchmark unloaded.
