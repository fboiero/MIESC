...
\midrule
\textbf{MIESC (latest reproducible local run)} & \textbf{22.1\%} & \textbf{95.1\%} & \textbf{35.9\%} \\
\bottomrule
\end{tabular}
...
Table~\ref{tab:smartbugs} shows the latest full-corpus reproducible SmartBugs profile stored in the repository as \texttt{paper1\_smartbugs\_eval\_layers\_1\_6\_7.json}. MIESC reaches 95.1\% recall and 35.9\% F1, substantially above the single-tool baselines reported by Durieux et al.~\cite{durieux2020empirical}. This benchmark is useful for classical vulnerability patterns, but it is not the central evidence for business-logic vulnerabilities; that role is played by EVMBench (Section~\ref{sec:evmbench}).
...
The results show a clear split between classical pattern detection and semantic vulnerability detection. On SmartBugs-curated, MIESC's latest full-corpus local profile reaches 95.1\% recall after selecting Slither's \texttt{solc} binary from the contract pragma and activating Layer~6 specialized benchmark detectors instead of relying only on the previous layer mix. The remaining misses concentrate in heterogeneous or poorly represented categories such as ``other'' and access control. On EVMBench, static-only analysis reaches 18.3\% recall, while LLM providers detect complementary subsets of business-logic vulnerabilities and the four-provider union closes 18 otherwise missed findings beyond the best single provider. The key contribution is therefore not merely adding more tools, but using each layer for the vulnerability class it is suited to: static tools for known patterns, specialized detectors for domain-specific signals, and multi-provider LLMs for semantic reasoning.
