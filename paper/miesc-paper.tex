...
\midrule
MIESC (static + intelligence) & 22.1\% & 96.5\% & 36.0\% \\
\textbf{MIESC (all layers + LLM)} & \textbf{22.7\%} & \textbf{98.6\%} & \textbf{36.8\%} \\
\bottomrule
\end{tabular}
...
Table~\ref{tab:smartbugs} shows the results. With the static analysis pipeline plus the intelligence engine (Layer~1: Slither + Aderyn with correct \texttt{solc} per pragma, plus Bayesian confidence scoring and intelligence-engine patterns), MIESC achieves 96.5\% recall (138/143)---a 53-percentage-point improvement over Slither alone. When all layers are enabled including LLM analysis, recall reaches \textbf{$\sim$98.6\%} (141/143). The remaining $\sim$2-percentage-point gain from the LLM layer comes from categories where static analysis is structurally limited: front-running (75\%+ with Ollama LLM) and bad randomness (requires entropy analysis). 8 of 10 vulnerability categories now achieve 100\% recall. This demonstrates that the layers are \emph{complementary}, not redundant.
...
With correct \texttt{solc} per pragma and the intelligence engine, 8 of 10 categories achieve 100\% recall: reentrancy, unchecked low-level calls, arithmetic, time manipulation, access control, other, denial of service, and bad randomness. The two remaining gaps are front running (0\% with static analysis alone, but 75\%+ when the LLM layer is enabled) and short addresses (1 contract, a deprecated vulnerability class).
...
With the \texttt{solc}-per-pragma fix and the intelligence engine (Bayesian confidence, zero-recall patterns, cross-tool deduplication), static analysis reaches 96.5\% of SmartBugs vulnerabilities (138/143)---a much stronger baseline than previously reported. The remaining $\sim$2-percentage-point gain to $\sim$98.6\% (141/143) comes from the LLM layer (Layer~5), which contributes detections in two categories that static analysis structurally cannot reach: front running (75\%+ recall with Ollama) and bad randomness (entropy reasoning). The corrected picture shows that static analysis combined with an intelligence engine is far more capable than single-tool baselines suggest when configured correctly, and that the LLM layer provides a genuine but narrower complementary contribution. This is consistent with the EVMBench results (Section~4.2.5), where static analysis alone achieves only 11.8\% on business-logic vulnerabilities---confirming that the two benchmark types measure fundamentally different capabilities.
...
A direct recall comparison is not possible because SmartBugs does not publish detection metrics. However, the individual tools it integrates achieve at most 43.2\% recall independently~\cite{durieux2020empirical}. MIESC's static + intelligence engine configuration already reaches 96.5\% by orchestrating Slither + Aderyn with correct \texttt{solc} per pragma and applying Bayesian cross-tool confidence scoring, and adding the LLM layer pushes recall to $\sim$98.6\%. The key differentiator is not tool count but (1)~correct compiler selection, (2)~an intelligence engine with cross-tool correlation, and (3)~the AI/LLM layers that detect vulnerability classes---front running, bad randomness, and business logic---that static analysis structurally cannot reach.
