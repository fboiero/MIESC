...
The results show a clear split between classical pattern detection and semantic vulnerability detection. On SmartBugs-curated, MIESC's latest full-corpus local profile reaches 95.8\% recall after selecting Slither's \texttt{solc} binary from the contract pragma and activating Layer~6 specialized benchmark detectors instead of relying only on the previous layer mix. The remaining misses concentrate in heterogeneous or poorly represented categories such as ``other'' and access control. On EVMBench, static-only analysis reaches 18.3\% recall, while LLM providers detect complementary subsets of business-logic vulnerabilities and the four-provider union closes 18 otherwise missed findings beyond the best single provider. The key contribution is therefore not merely adding more tools, but using each layer for the vulnerability class it is suited to: static tools for known patterns, specialized detectors for domain-specific signals, and multi-provider LLMs for semantic reasoning.
