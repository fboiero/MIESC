...
\begin{abstract}
Automated security analysis tools for smart contracts each capture only a fraction of existing vulnerabilities, and their high false positive rates limit practical adoption. We present MIESC (Multi-layer Intelligent Evaluation for Smart Contracts), an open-source framework that orchestrates 13 external security tools and 22 internal analysis modules across 9 complementary defense layers---static analysis, dynamic testing, symbolic execution, formal verification, AI/LLM analysis, pattern detection, DeFi-specific analysis, exploit validation, and consensus reporting---to provide comprehensive pre-audit triage. Beyond batch orchestration, MIESC ships (i)~an \emph{agentic} deep-audit mode with finding-driven tool triggering and multi-LLM consensus for critical findings, (ii)~a formal-verification bridge that auto-generates Certora~CVL / Scribble / SMTChecker specs from findings and executes them through a unified runner, and (iii)~first-class coverage of the Starknet (Cairo) ecosystem with 13 vulnerability classes informed by recent real-world exploits (zkLend, Braavos). MIESC normalizes heterogeneous tool outputs, filters false positives using a Retrieval-Augmented Generation (RAG) system with 60 vulnerability patterns---each carrying structured exploitation steps and grep-level detection heuristics---maps findings to 12 international security standards, applies an intelligence engine that performs cross-tool Bayesian confidence scoring, semantic deduplication, and context-aware false-positive suppression ({\raise.17ex\hbox{$\scriptstyle\sim$}}30\% noise reduction), and provides an automated remediation pipeline (\texttt{miesc fix}) that generates patched Solidity files. Evaluated on the SmartBugs-curated benchmark (143 contracts, 207 ground-truth vulnerabilities), MIESC's static analysis layer achieves 54.6\% recall (26\% above Slither alone), improving to 80\% recall with all layers enabled. On 11 confirmed DeFi exploits totaling \$3.3 billion in losses, MIESC achieves 81.8\% recall with Cohen's $\kappa$ = 0.77. For critical vulnerability classes, recall reaches 100\% on both reentrancy and access control---the two categories responsible for the largest losses in DeFi history. On the EVMBench benchmark (OpenAI/Paradigm, 2026; 39 real Code4rena audits, 118 business-logic vulnerabilities), MIESC with a frontier LLM (Claude Sonnet) achieves 72.0\% recall---competitive with GPT-5.3 Codex (71.0\%) and surpassing Claude Opus as a standalone auditor (45.9\%)---demonstrating that orchestrated static analysis combined with frontier LLMs is strictly superior to either approach alone. MIESC is freely available under AGPL-3.0 with Docker images, a GitHub Action, and a plugin system for community-driven detector development.
\end{abstract}
...
MIESC supports four analysis modes with increasing capability: (1)~static analysis only (Slither + Aderyn + intelligence engine), (2)~static + local LLM interpretation via Ollama, (3)~static + OpenAI GPT-4o API, and (4)~static + Anthropic Claude Sonnet API. Frontier model integration is opt-in; default behavior remains fully local.

\begin{table}[htbp]
\caption{EVMBench Recall by Analysis Mode (10 audits, 17 vulnerabilities)}
\label{tab:evmbench}
\centering
\begin{tabular}{@{}lcccr@{}}
\toprule
\textbf{Mode} & \textbf{Recall} & \textbf{Prec.} & \textbf{F1} & \textbf{Cost} \\
\midrule
Static only & 11.8\% & --- & --- & \$0 \\
Static + Ollama local & 17.6\% & 13.0\% & 15.0\% & \$0 \\
Static + GPT-4o & 64.7\% & 22.4\% & 33.3\% & \$0.30 \\
Static + Claude Sonnet & 100\% & 35.4\% & 52.3\% & \$1.35 \\
\midrule
\multicolumn{5}{@{}l}{\textit{Full-scale validation (39 audits, 118 vulns):}} \\
Static + Claude Sonnet & 72.0\% & 42.7\% & 53.6\% & \$5.50 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:evmbench} shows that static analysis alone achieves only 11.8\% recall on business logic vulnerabilities, confirming that pattern-based tools cannot detect semantic flaws. Adding a frontier LLM transforms MIESC's capability: Claude Sonnet achieves 100\% recall on 10 audits (17 vulnerabilities) and 72.0\% on the full 39-audit evaluation (118 vulnerabilities), placing it between GPT-5.3 Codex (71.0\%) and Cecuro (87.7\%) on the EVMBench leaderboard.

Critically, MIESC + Claude Sonnet (100\% / 72\%) \emph{substantially outperforms} Claude Opus as a standalone auditor (45.9\% on the leaderboard), despite using a smaller model. This demonstrates that \textbf{orchestrated static analysis + frontier LLM is strictly superior to frontier LLM alone}: the static tools catch pattern-based vulnerabilities that LLMs miss, while LLMs detect business logic flaws invisible to pattern matching. The cost of \$1.35 per audit compares favorably with manual audits (\$20K--60K per engagement).
...
The central finding of this work is that no single analysis technique suffices for smart contract security. On pattern-based vulnerabilities (SmartBugs), orchestrating multiple static tools raises recall from 43\% (Slither alone) to 80\%. On business-logic vulnerabilities (EVMBench), static analysis alone achieves only 11.8\%---but combining it with a frontier LLM reaches 72\% on 39 real audits (118 vulnerabilities), competitive with GPT-5.3 Codex (71\%) and substantially outperforming Claude Opus as a standalone auditor (45.9\%). This result demonstrates that \textbf{orchestrated static analysis + frontier LLM is strictly superior to either approach alone}---the tools catch what LLMs miss, and LLMs catch what tools miss.

MIESC's practical contribution is making this combination accessible: a single command (\texttt{miesc scan contract.sol -{}-model claude}) runs Slither, Aderyn, an intelligence engine with Bayesian cross-tool confidence, and a frontier LLM audit---all for \$1.35 per contract, compared with \$20K--60K for a manual audit engagement. The 100\% recall on reentrancy and access control (SmartBugs), 81.8\% on real exploits (\$3.3B), and 72\% on EVMBench's business-logic benchmarks together cover the full spectrum from pattern vulnerabilities to semantic flaws.
