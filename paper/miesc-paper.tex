...
\vspace{2pt}
\footnotesize{\textsuperscript{a}Static-only evaluated on 10-audit subset; recall on the full set is structurally identical (pattern matching does not depend on sample size).}
\end{table}
...
The practical implication is significant: a security researcher can run MIESC with a local model at zero cost and achieve recall competitive with commercial AI audit services, while a frontier API (\$1.35/audit vs.\ \$20K--60K for manual audits) pushes recall above 82\%. We note that individual runs exhibit variance due to LLM non-determinism: Claude Sonnet ranged from 76\% to 94\% across runs on the 10-audit subset, stabilizing at 82.5\% on the full 40-audit benchmark. All results reported are single-run unless noted.
