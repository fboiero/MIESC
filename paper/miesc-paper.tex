...
\textbf{Dynamic testing.} Echidna~\cite{grieco2020echidna} performs property-based fuzzing using coverage-guided input generation. Foundry's \texttt{forge} combines unit testing with fuzz-based invariant checking. Medusa extends fuzzing with corpus-based strategies.
...
\textbf{Reproducibility.} All benchmark scripts, ground truth data, and evaluation methodology are included in the repository. Results can be reproduced with: \verb|python benchmarks/reproduce_benchmark.py --save|.
...
MIESC is available at \url{https://github.com/fboiero/MIESC} under AGPL-3.0.
