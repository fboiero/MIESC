...
\begin{abstract}
Automated smart contract security tools tend to specialize: static analyzers cover known code patterns, fuzzers exercise reachable states, and LLM-based auditors can reason about higher-level intent but vary across runs. This paper studies whether a reproducible orchestration of these techniques improves coverage for smart-contract triage. MIESC combines 35 analysis modules across 9 layers, normalizes their outputs, applies RAG-backed false-positive filtering, and records the artifacts needed to reproduce each reported claim. On SmartBugs-curated (143 contracts), the primary static+intelligence profile reports 95.8\% recall (137/143); a secondary local 32B follow-up over the remaining misses raises this to 97.9\% (140/143). On 11 real DeFi exploits (\$1.59B in evaluated losses), MIESC flags the responsible vulnerability class in 9/11 cases (81.8\%; 95\% CI [52\%, 95\%] by the Wilson method on this small sample). On an EVMBench local high-severity extraction (40 audits, 120 findings), a four-provider union ensemble detects 111/120 findings (92.5\% recall) versus 82.5\% for the best single provider and 59.2\% for a local 32B model; this is reported as a local extraction rather than an official-leaderboard score and is treated as secondary evidence. An MIT-licensed open-weight model (DeepSeek-reasoner) attains the best precision and F1 in this comparison at 15--69$\times$ lower per-audit cost, at lower single-run recall than the frontier providers. Precision is low because informational findings dominate, so MIESC is positioned as a reproducible pre-audit triage aid rather than a replacement for expert review. All datasets, scripts, and a per-claim matrix are released for independent reproduction; MIESC is available under AGPL-3.0.
\end{abstract}
...
Contributions~6--8 are implemented and described for completeness but are not separately benchmarked in this paper; the evaluation focuses on contributions~1--5. Unlike prior single-technique analyzers and benchmarking frameworks, MIESC unifies multi-tool static orchestration, RAG-backed false-positive filtering, and a multi-provider LLM ensemble under one normalized finding schema with reproducible per-claim evidence. MIESC is designed as a \emph{pre-audit triage tool}, not a replacement for manual expert review. By automating the multi-tool orchestration and normalization workflow, it reduces audit preparation time and enables continuous security monitoring in CI/CD pipelines; because the local-first profile keeps source code off third-party providers, it lowers the barrier to industrial adoption in regulated settings.
...
Table~\ref{tab:smartbugs} shows the latest full-corpus reproducible SmartBugs profile stored in the repository. MIESC reaches 95.8\% recall and 35.9\% F1, substantially above the single-tool baselines reported by Durieux et al.~\cite{durieux2020empirical}. This benchmark is useful for classical vulnerability patterns, but it is not the central evidence for business-logic vulnerabilities; that role is played by EVMBench (Section~\ref{sec:evmbench}).
...
Acknowledgments are omitted in this anonymized version for double-blind review. The authors thank the Ethereum security community and the developers of Slither, Mythril, Echidna, Foundry, Aderyn, and Halmos for their foundational contributions.

\subsection*{Data Availability}

MIESC source code, benchmark scripts, ground truth datasets, and evaluation results are publicly available at the project repository (URL withheld for double-blind review) under AGPL-3.0. The SmartBugs-curated dataset is from Durieux et al.~\cite{durieux2020empirical}. The real-world exploit dataset was curated from DeFiHackLabs (\url{https://github.com/SunWeb3Sec/DeFiHackLabs}). Paper-specific reproducibility artifacts are documented in \texttt{PAPER1\_REPRODUCIBILITY.md}, with generated outputs \texttt{paper1\_claims\_matrix.json}, \texttt{evmbench\_static\_40.json}, \texttt{evmbench\_ensemble\_40.json}, and \texttt{paper1\_smartbugs\_local\_ollama\_followup\_20260623.json}.
