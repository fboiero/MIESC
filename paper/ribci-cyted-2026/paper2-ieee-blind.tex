...
\begin{enumerate}
    \item \textbf{Self-contained patch generation} (\texttt{miesc fix}): text-level Solidity patch candidates that avoid adding external dependencies. Evaluated on 143 contracts under the v2 external-validation baseline, the most conservative result is a 57\% clean-HIGH rate under independent Slither validation (70/123); the intermediate metrics are 100\% standalone compilation for patched contracts, 86\% current-scan fix application (123/143 after 18 no-HIGH and 2 empty-scan cases), 72\% target-finding elimination by re-scan, and 98\% under a bounded no-regression criterion. We foreground the external clean-HIGH rate because re-scanning with the detecting pipeline is circular.
...
\begin{table}[htbp]
\caption{Feature comparison with prior repair tools. T/S/C~= generates exploit tests / formal specs / compliance mapping. \textsuperscript{c}Fix\% and Comp.\% use each tool's own definition and scope and are not directly comparable; shown for context, not ranking.}
...
\textbf{MIESC} & '26 & 10 cats. & 72\textsuperscript{c} & 100 & \checkmark & \checkmark/\checkmark \\
\bottomrule
\end{tabular}
...
MIESC is available under AGPL-3.0 at the project repository (URL withheld for double-blind review).

% ===========================================================================
\section*{Acknowledgment}
% ===========================================================================

Acknowledgments are omitted in this anonymized version for double-blind review. The authors thank the Ethereum security community and the developers of Foundry, Slither, and OpenZeppelin.
