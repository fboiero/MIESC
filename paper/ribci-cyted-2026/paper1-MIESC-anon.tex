...
\textbf{Static analysis.} Slither~[2] uses intermediate representation (SlithIR) to detect vulnerability patterns with low false positive rates. Aderyn~[6] implements Rust-based AST analysis for high performance. Solhint provides Solidity-specific linting rules.

\textbf{Dynamic testing.} Echidna~[4] performs property-based fuzzing using coverage-guided input generation. Foundry's forge tool combines unit testing with fuzz-based invariant checking. Medusa extends fuzzing with corpus-based strategies.

\textbf{Symbolic execution.} Mythril~[3] explores execution paths using SMT solving to find reachable vulnerabilities. Halmos~[7] enables symbolic testing within the Foundry framework. Manticore~[8] combines symbolic execution with concrete execution.
...
\includegraphics[width=\linewidth]{figs/fig1-1.png}
...
Beyond batch orchestration, MIESC includes a \emph{DeepAuditAgent} that implements an autonomous 4-phase investigation loop inspired by intelligent agent architectures~[15]. Agents communicate via a Model Context Protocol (MCP) bus~[16], enabling modular coordination:
...
\textbf{Environment}: macOS ARM64 (Apple Silicon), 48GB unified memory. The SmartBugs artifacts reported here were produced with MIESC v5.4.0/v5.4.1 and Python 3.14.4 on 2026-05-06 through 2026-05-09, then consolidated in the Paper~1 v2 evidence freeze on 2026-06-23. EVMBench provider runs use stored MIESC result artifacts; the reproducible ensemble artifact is generated by \texttt{generate\_paper1\_artifacts.py}, which computes a deterministic union over matched \texttt{audit:vulnerability} keys.
...
\vspace{2pt}
\footnotesize{\textsuperscript{*}Baseline from Durieux et al.~[5]}
\end{table}

Table~\ref{tab:smartbugs} shows the latest full-corpus reproducible SmartBugs profile stored in the repository as \texttt{paper1\_smartbugs\_eval\_layers\_1\_6\_7.json}. MIESC reaches 95.8\% recall and 35.9\% F1, substantially above the single-tool baselines reported by Durieux et al.~[5]. This benchmark is useful for classical vulnerability patterns, but it is not the central evidence for business-logic vulnerabilities; that role is played by EVMBench (Section~\ref{sec:evmbench}).
...
MIESC achieved 81.8\% recall (9/11) on this corpus, with a 95\% confidence interval of [52\%, 95\%] by the Wilson method (n=11). Given the small sample size, we report this as indicative rather than definitive. Notable detections include the Euler Finance reentrancy (\$197M), Ronin Bridge access control (\$624M), and Parity wallet freeze (\$280M).

\subsubsection{Execution Performance}

Table~\ref{tab:performance} summarizes MIESC's execution speed for the reproducible SmartBugs full-corpus profile. The run completed all 143 contracts in 737 seconds, or 11.6 contracts per minute.
...
+ DeepSeek-reasoner (open weight) & 70.8\% & \textbf{88.5\%} & \textbf{78.7\%} & \$0.08\textsuperscript{b} & 40 \\
...
\vspace{2pt}
\footnotesize{\textsuperscript{a}GPT-4o artifact contains one clone-failed audit entry; recall is 87/118. Costs are approximate API spend observed for these single stored runs; static and local Ollama require no API calls. Ensemble result is generated by \texttt{benchmarks/generate\_paper1\_artifacts.py} as the union over \texttt{audit:vulnerability} keys. \textsuperscript{b}DeepSeek-reasoner (MIT open-weight) is a single stored run through the same \texttt{evmbench\_eval.py} harness and version (5.3.1) as the other providers, and is therefore directly comparable; its cost is estimated from DeepSeek's \$0.55/\$2.19 per-MTok pricing because the harness does not capture per-call adapter tokens.}
\end{table}

Table~\ref{tab:evmbench} separates three observations. First, static-only MIESC reaches 18.3\% recall on EVMBench, which is useful as a baseline but insufficient for business-logic flaws. Second, a local 32B model (qwen2.5-coder:32b via Ollama) reaches 59.2\% recall without API calls, showing that local models are a viable low-cost triage option but not a replacement for stronger reviewers. Third, the best single provider (Claude Sonnet 4.6, 82.5\% recall at \$5.50/audit) and the four-provider union (\textbf{111/120}, \textbf{92.5\%}) show complementary coverage. The union is numerically above the 87.7\% recall publicly reported by Cecuro, but our local extraction and matcher differ from the official EVMBench judge; we therefore treat this as contextual evidence, not a formal leaderboard result.
...
For deployment, the useful distinction is cost and data exposure. A security researcher can run MIESC with a local model and keep the code offline, accepting the lower 59.2\% recall. A frontier provider improves recall to 82.5\% in the stored run, and the full union improves it further, at higher cost and with provider exposure. Individual LLM runs exhibit variance (about $\pm$4pp on 40 audits in our observations), so all EVMBench results are reported as stored single-run artifacts.

\textbf{Open-weight models.} DeepSeek-reasoner, an MIT-licensed open-weight model accessed through its hosted API, reaches 70.8\% single-run recall---below the frontier providers---yet posts the highest precision (88.5\%) and F1 (78.7\%) in Table~\ref{tab:evmbench}, because it reports far fewer findings (96 vs.\ 218--434 for the frontier providers) and thus fewer false positives. Its estimated \$0.08/audit is 15--69$\times$ below the frontier per-audit cost. Because the model is cheap, extra runs are affordable: a three-run self-ensemble (the union of three independent DeepSeek single runs, $\approx$\$0.24/audit) raises recall to 81.7\% (98/120), which falls within the frontier \emph{single-run} range (73.7--82.5\%). We report the single run as the like-for-like, run-count-matched comparison against the frontier single runs, and the self-ensemble as a cost-controlled variance-reduction option---not as a run-count-matched claim. The broader point is one of openness tiers: paying frontier prices buys higher raw single-run recall, but an open-weight model delivers the best precision/F1 at 15--69$\times$ lower cost, and closes the recall gap when its low cost is spent on additional runs.
...
\subsection{Open-weight sovereignty and cost efficiency}

The EVMBench provider comparison (Table~\ref{tab:evmbench}) carries a consequence beyond recall. The open-weight DeepSeek-reasoner (MIT-licensed weights) attains the highest single-run precision (88.5\%) and F1 (78.7\%) in the comparison at an estimated \$0.08/audit---15--69$\times$ below the frontier per-audit cost---trailing the frontier only on single-run recall (70.8\% vs.\ 73.7--82.5\%), a gap a low-cost self-ensemble narrows. This matters for a reproducibility-oriented pipeline: a versioned open-weight model executed under the analyst's control is a freezable, inspectable artifact, whereas a proprietary API is a moving target that can change or be deprecated between runs, undermining the reproducibility this paper otherwise emphasizes. The result reframes the open-source choice from a capability sacrifice into a defensible default---best precision/F1 at a fraction of the cost, with full data control---while we report honestly that the strongest open-weight result was obtained via a hosted API and that frontier models retain a single-run recall advantage.
...
This generates \texttt{evmbench\_ensemble\_40.json} and \texttt{paper1\_claims\_matrix.json}. SmartBugs can be reproduced with the command listed in \texttt{PAPER1\_REPRODUCIBILITY.md}; the fixed full-corpus profile is \texttt{miesc evaluate corpus ... -{}-layers 1,6,7 -{}-timeout 120}, and the local-Ollama lift is recorded in the v2 SmartBugs follow-up JSON named there. The same document records the full-tool smoke command for layers 1--9. The Slither adapter selects an installed \texttt{solc-select} artifact compatible with each contract pragma (for example, \texttt{\^{}0.4.x} contracts use \texttt{solc-0.4.26}), avoiding the historical failure mode where \texttt{solc 0.8.20} was applied to legacy Solidity.
...
MIESC source code, benchmark scripts, ground truth datasets, and evaluation results are publicly available at the project repository (link withheld for double-blind review) under AGPL-3.0. The SmartBugs-curated dataset is from Durieux et al.~[5]. The real-world exploit dataset was curated from DeFiHackLabs (\url{https://github.com/SunWeb3Sec/DeFiHackLabs}). Paper-specific reproducibility artifacts are documented in \texttt{PAPER1\_REPRODUCIBILITY.md}, with generated outputs \texttt{paper1\_claims\_matrix.json}, \texttt{evmbench\_static\_40.json}, \texttt{evmbench\_ensemble\_40.json}, and \texttt{paper1\_smartbugs\_local\_ollama\_followup\_20260623.json}.

% \balance
\section*{References}
\begin{enumerate}
\item N.~Atzei, M.~Bartoletti, and T.~Cimoli, ``A survey of attacks on {Ethereum} smart contracts ({SoK}),'' \emph{Proceedings of the 6th International Conference on Principles of Security and Trust (POST)}, pp. 164--186, 2017.
\item J.~Feist, G.~Grieco, and A.~Groce, ``Slither: A static analysis framework for smart contracts,'' in \emph{2019 IEEE/ACM 2nd International Workshop on Emerging Trends in Software Engineering for Blockchain (WETSEB)}. IEEE, 2019, pp. 8--15.
\item B.~Mueller, ``Smashing {Ethereum} smart contracts for fun and real profit,'' in \emph{HITB Security Conference}, 2018.
\item G.~Grieco, W.~Song, A.~Cygan, J.~Feist, and A.~Groce, ``Echidna: Effective, usable, and fast fuzzing for smart contracts,'' in \emph{Proceedings of the 29th ACM SIGSOFT International Symposium on Software Testing and Analysis (ISSTA)}. ACM, 2020, pp. 557--560.
\item T.~Durieux, J.~F. Ferreira, R.~Abreu, and P.~Cruz, ``Empirical review of automated analysis tools on 47,587 {Ethereum} smart contracts,'' in \emph{Proceedings of the ACM/IEEE 42nd International Conference on Software Engineering (ICSE)}. ACM, 2020, pp. 530--541.
\item {Cyfrin}, ``Aderyn: Rust-based {Solidity} ast analyzer,'' \url{https://github.com/Cyfrin/aderyn}, 2024.
\item {a16z crypto}, ``Halmos: Symbolic testing for {EVM} smart contracts,'' \url{https://github.com/a16z/halmos}, 2023.
\item M.~Mossberg, F.~Manzano, E.~Hennenfent, A.~Groce, G.~Grieco, J.~Feist, T.~Brunson, and A.~Dinaburg, ``Manticore: A user-friendly symbolic execution framework for binaries and smart contracts,'' in \emph{2019 34th IEEE/ACM International Conference on Automated Software Engineering (ASE)}. IEEE, 2019, pp. 1186--1189.
\item J.~F. Ferreira, P.~Cruz, T.~Durieux, and R.~Abreu, ``{SmartBugs}: A framework to analyze {Solidity} smart contracts,'' in \emph{Proceedings of the 35th IEEE/ACM International Conference on Automated Software Engineering (ASE)}. ACM, 2020, pp. 1349--1352.
\item M.~di~Angelo, T.~Durieux, J.~F. Ferreira, and G.~Salzer, ``{SmartBugs} 2.0: An execution framework for weakness detection in {Ethereum} smart contracts,'' in \emph{Proceedings of the 38th IEEE/ACM International Conference on Automated Software Engineering (ASE)}. IEEE, 2023, pp. 2102--2105.
\item Y.~Sun, D.~Wu, Y.~Xue, H.~Liu, H.~Wang, Z.~Xu, X.~Xie, and Y.~Liu, ``{GPTScan}: Detecting logic vulnerabilities in smart contracts by combining {GPT} with program analysis,'' in \emph{Proceedings of the IEEE/ACM 46th International Conference on Software Engineering (ICSE)}. ACM, 2024, pp. 1--13.
\item A.~Gupta, V.~Gottimukkala, D.~Robinson, Paradigm, and OpenAI, ``{EVMBench}: Evaluating {AI} agents on smart contract security,'' \emph{arXiv preprint arXiv:2603.04915}, 2026. [Online]. Available: \url{https://github.com/paradigmxyz/evmbench}
\item {Cecuro}, ``{AI} audit firm cecuro outperforms nearest rival by 2x on {OpenAI} smart contract exploit benchmark,'' \url{https://chainwire.org/2026/04/16/ai-audit-firm-cecuro-outperforms-nearest-rival-by-2x-on-openai-smart-contract-exploit-benchmark/}, 2026, press release; vendor-reported result, not a peer-reviewed evaluation.
\item J.~H. Saltzer and M.~D. Schroeder, ``The protection of information in computer systems,'' \emph{Proceedings of the IEEE}, vol.~63, no.~9, pp. 1278--1308, 1975.
\item M.~Wooldridge and N.~R. Jennings, ``Intelligent agents: Theory and practice,'' \emph{The Knowledge Engineering Review}, vol.~10, no.~2, pp. 115--152, 1995.
\item {Anthropic}, ``Model context protocol specification,'' \url{https://modelcontextprotocol.io}, 2024.
\end{enumerate}
