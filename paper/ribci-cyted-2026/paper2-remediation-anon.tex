...
The smart contract security ecosystem has invested heavily in \emph{detection}---static analyzers~[1], fuzzers~[2], symbolic executors~[3], and more recently LLM-based auditors~[4]---but comparatively little in what happens \emph{after} a vulnerability is found. In practice, the workflow looks like this:
...
\includegraphics[width=\linewidth]{figs/fig2-1.png}
...
Access Control & 17 & 17 (100\%) & 10 (59\%) & 17 (100\%) \\
Unchecked Calls & 47 & 47 (100\%) & 34 (72\%) & 47 (100\%) \\
Arithmetic & 4 & 4 (100\%) & 3 (75\%) & 4 (100\%) \\
Others (6 cats) & 24 & 24 (100\%) & 11 (46\%) & 24 (100\%) \\
\midrule
\textbf{Total} & \textbf{123} & \textbf{123 (100\%)} & \textbf{88 (72\%)} & \textbf{121 (98\%)} \\
\bottomrule
\end{tabular}
...
Viewed by transformation class, reentrancy guard injection applies to 31 contracts, compiles in 31, and eliminates the target finding in 30. The \texttt{require(success)} wrapper applies to 47 unchecked-call contracts, compiles in 47, and eliminates 34 target findings. External Slither validation is stricter than the MIESC re-scan: 70/123 patched contracts are clean of HIGH findings, while 53 residual HIGH findings remain, concentrated in reentrancy, unchecked-call, and dynamic-array patterns.

\textbf{Experiment validity audit.} To separate patch quality from corpus compilability, we additionally compile the original contracts under the same standalone criterion. In the v2 run, 142/143 original files compile standalone. The patcher emits 123 patched contracts; all 123 compile, and all 123 are also checked by external Slither validation without tool errors. Joint success is stricter than any single metric: 88/123 patches both compile and reduce the target finding under MIESC re-scan, 121/123 compile and satisfy the bounded no-regression criterion, and 70/123 compile and are clean of HIGH findings under Slither.
...
\midrule
SCRepair~[5] & '20 & GP & --- & --- & Req\textsuperscript{a} & \texttimes/\texttimes \\
sGuard~[6] & '21 & 4 & --- & ${\sim}$100 & \texttimes & \texttimes/\texttimes \\
Elysium~[7] & '22 & 7 SWC & +30\textsuperscript{b} & --- & \texttimes & \texttimes/\texttimes \\
ACFIX~[8] & '24 & 1 & 94.9 & --- & \texttimes & \texttimes/\texttimes \\
SCPatcher~[9] & '25 & Gen. & 81.5 & 91 & \texttimes & \texttimes/\texttimes \\
\textbf{MIESC} & \textbf{'26} & \textbf{10 cats.} & \textbf{72} & \textbf{100} & \checkmark & \checkmark/\checkmark \\
\bottomrule
\end{tabular}
...
Table~\ref{tab:comparison} compares MIESC against five prior automated repair tools. MIESC reaches 100\% standalone compilation for emitted patches (123/123) under a strict no-external-dependency criterion, while SCPatcher reports 91\% compilation under different evaluation targets and dependency assumptions, so the numbers are not directly interchangeable; the MIESC ``Fix\%'' column reports vulnerability elimination on re-scan (88/123, 72\%). ACFIX reports 94.9\% fix rate but is limited to a single vulnerability class (RBAC). MIESC is the only evaluated tool that reports artifacts across the remediation lifecycle: patch generation, exploit testing, formal specifications, and compliance mapping.

\subsection{Failure Analysis}

In the v2 baseline, standalone compilation is no longer a failure mode: all 123 emitted patches compile (0 failures), versus 51/141 standalone failures in the prior v1 run (42 undefined-symbol and 9 other compiler errors, with unchecked low-level calls the largest share). The inline, dependency-free guards and the current-scan-filtered denominator removed that bottleneck. The remaining v2 gaps are therefore semantic rather than syntactic: 35/123 patched contracts do not eliminate the target HIGH/CRITICAL finding on MIESC re-scan, and external Slither validation finds only 70/123 patched contracts clean of HIGH (57\%), reporting 53 residual HIGH findings across the rest.

This reframes the root-cause direction for future work. With standalone compilation solved, the priority shifts from symbol-resolution repair to semantic adequacy: eliminating the target finding without introducing new HIGH findings under independent (Slither/SMTChecker/Foundry) validation. The external-validation gap (53 residual HIGH findings) is the main lever, and removing re-scan circularity by leaning on independent tools is the next step.
...
\textbf{External validity.} (3)~\emph{Corpus selection bias}: SmartBugs-curated consists primarily of contracts from 2017--2019 with known vulnerabilities. These contracts are simpler (avg.\ 85 LoC) and more heavily studied than production DeFi protocols (avg.\ 500+ LoC with complex inheritance). Results on production code may differ, particularly for the compilation rate (which depends heavily on dependency availability). (4)~\emph{Vulnerability distribution}: the corpus over-represents reentrancy (31 contracts) and unchecked low-level calls (52 contracts) relative to their frequency in production code. Categories like oracle manipulation, MEV, and flash loan attacks---which dominate real-world exploits~[17]---are absent from SmartBugs.
...
\textbf{Mitigation strategy.} We address threats (3) and (4) by additionally reporting EVMBench results (120 business-logic vulnerabilities from real audit reports) in our companion paper~[14], where MIESC achieves 92.5\% recall on a corpus that includes oracle manipulation, flash loans, and governance attacks.

% ===========================================================================
\section{Conclusion and Future Work}
% ===========================================================================

This paper reports how far the current MIESC remediation pipeline can move after detection, and where it still falls short. Evaluated on 143 contracts from SmartBugs-curated, the v2 baseline applies fixes to 123 contracts under the current scan (86\%), compiles all 123 emitted patches standalone (100\%), eliminates the target-category finding in 88/123 cases by MIESC re-scan (72\%), satisfies the bounded no-regression criterion in 121/123 cases (98\%), and reports 70/123 patched contracts clean of HIGH findings under external Slither validation (57\%). The 57\% external clean-HIGH result is intentionally reported as a separate metric: it shows that successful patch emission and compilation do not imply semantic closure under an independent analyzer. Compared to five prior repair tools (Table~\ref{tab:comparison}), MIESC is the only system in our comparison that reports exploit tests, formal specifications, and compliance documentation alongside patches.
...
\item X.~Yu, H.~Zhao, B.~Hou, Z.~Ying, and B.~Shi, ``Smart contract repair,'' \emph{ACM Transactions on Software Engineering and Methodology (TOSEM)}, vol.~29, no.~4, pp. 1--32, 2020.
\item T.~D. Nguyen, L.~H. Pham, J.~Sun, Y.~Lin, and Q.~T. Minh, ``{sGuard}: Towards fixing vulnerable smart contracts automatically,'' in \emph{2021 IEEE Symposium on Security and Privacy (SP)}. IEEE, 2021, pp. 1349--1367.
\item M.~Rodler, W.~Li, G.~O. Karame, and L.~Davi, ``Elysium: Context-aware bytecode-level patching to automatically heal vulnerable smart contracts,'' in \emph{Proceedings of the 31st ACM SIGSOFT International Symposium on Software Testing and Analysis (ISSTA)}. ACM, 2022, pp. 742--754.
\item L.~Zhang, K.~Li, K.~Sun, D.~Wu, Y.~Liu, H.~Tian, and Y.~Liu, ``{ACFIX}: Guiding {LLMs} with mined common {RBAC} practices for context-aware repair of access control vulnerabilities in smart contracts,'' \emph{IEEE Transactions on Software Engineering}, vol.~50, no.~7, pp. 1837--1853, 2024.
\item S.-L. Li, Y.-T. Chen, and W.-C. Huang, ``{SCPatcher}: Automated smart contract patching via {RAG}-enhanced {LLM},'' \emph{arXiv preprint arXiv:2604.00687}, 2025.
\item {Paradigm}, ``Foundry: {Ethereum} development toolkit,'' \url{https://github.com/foundry-rs/foundry}, 2025.
\item V.~W{\"u}stholz and M.~Christakis, ``Harvey: A greybox fuzzer for smart contracts,'' \url{https://arxiv.org/abs/2005.03350}, 2020.
...
\item Anonymous Author(s), ``{MIESC}: Multi-layer intelligent evaluation for smart contracts,'' \emph{arXiv preprint}, 2026.
