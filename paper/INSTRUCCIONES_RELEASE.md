...
El package en PyPI está en v5.0.3, hay que actualizar a 5.1.1.
...
python3 -m twine upload dist/miesc-5.1.1*
```
...
python3 -m twine upload --username __token__ --password pypi-XXXXX dist/miesc-5.1.1*
```

Verificar después:
```bash
pip install miesc==5.1.1
miesc --version  # debe decir 5.1.1
```

## 2. Docker: Retagear latest en GHCR

La imagen `ghcr.io/fboiero/miesc:5.1.1` existe pero `latest` no apunta a ella.
...
docker tag ghcr.io/fboiero/miesc:5.1.1 ghcr.io/fboiero/miesc:latest
...
# Debe decir MIESC version 5.1.1
```
...
|---|--------|---------|----------|
| 1 | Publicar PyPI 5.1.1 | ALTO — pip install funciona | 5 min |
