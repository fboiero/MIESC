...
The test's assertion (\texttt{assertGt(balance, 1 ether)}) passes on the vulnerable contract (confirming drain) and fails on the patched version (guard blocks re-entry). This dual-assertion pattern---pass-on-vulnerable, fail-on-fixed for security properties---is automatically generated for each vulnerability category.

\subsection{Stage 4: Formal Verification}

The formal verification stage applies the Solidity compiler's built-in SMTChecker using the Constrained Horn Clauses (CHC) engine~\cite{halmos2023}. Unlike fuzzers that generate concrete test inputs, CHC-based verification exhaustively proves properties over all possible execution paths via fixed-point computation over the contract's control-flow graph.
...
After patching, the first two warnings disappear entirely (the reentrancy guard makes the re-entry path unreachable), and the third is resolved by the \texttt{require(success)} wrapper. This demonstrates that CHC verification can serve as an automated proof that the patch eliminates the vulnerability across \emph{all} inputs---not just the inputs exercised by the exploit test.
...
\begin{enumerate}
    \item \textbf{No code restructuring}: the patcher cannot reorder statements to implement the Checks-Effects-Interactions (CEI) pattern---the gold standard for reentrancy prevention. Instead, it adds a mutex guard, which is correct but stylistically different from what a human auditor would write.
    \item \textbf{Regex-based function matching}: unusual formatting (multi-line signatures, Unicode comments, preprocessor directives) can cause mismatched insertions. In our evaluation, this affected 3 of 125 contracts (2.4\%).
...
These limitations represent a fundamental tradeoff: text-level patching achieves 87\% fix application rate across \emph{all} Solidity versions (0.4--0.8) without requiring compilation, while AST-level approaches like sGuard achieve higher precision but are limited to specific compiler versions.

\subsection{Compilation Rate Analysis}

The 63\% compilation rate deserves scrutiny. We observe that it is primarily a property of the \emph{benchmark corpus}, not the patching algorithm: SmartBugs-curated contains deployed mainnet contracts that import external libraries (OpenZeppelin, custom utility contracts, proxy patterns). These contracts were never designed for standalone compilation. When we restrict the evaluation to \emph{self-contained} contracts (no external imports), the compilation rate rises to 88\% (52/59 contracts). This suggests that in the common development workflow---where dependencies are available via npm/forge---the effective compilation rate would be substantially higher.
...
Importantly, the exploit tests serve a dual role: (1)~they \emph{validate} the detection (if the test cannot trigger the vulnerability, the finding may be a false positive), and (2)~they \emph{verify} the fix (if the test still fails after patching, the fix is incomplete). This bidirectional feedback loop is unique to MIESC among the compared tools.

\subsection{Cost and Scalability}

The pipeline's execution cost depends on the LLM provider:
\begin{itemize}
    \item \textbf{Local (Ollama)}: \$0 per contract, $\sim$45 seconds on Apple Silicon M5 Pro with \texttt{qwen2.5-coder:32b}.
    \item \textbf{Frontier API}: \$0.18 per contract average (Claude Sonnet 4.6 at \$3/M input, \$15/M output tokens; typical contract uses $\sim$4K input + 8K output tokens).
    \item \textbf{Ensemble (3 providers)}: \$0.42 per contract with deduplication across Claude + GPT-4o + Ollama.
\end{itemize}

At these costs, scanning an entire protocol (50--200 contracts) costs \$9--84 with frontier APIs, or \$0 with local models at a modest accuracy tradeoff (local models achieve $\sim$78\% recall vs.\ 96.5\% with frontier ensemble).

\subsection{Towards Continuous Security}

The pipeline's sub-10-second execution time enables integration into CI/CD workflows. A GitHub Action running \texttt{miesc scan -{}-ci} on every pull request, combined with \texttt{miesc fix -{}-dry-run} to suggest patches in PR comments, would provide continuous security feedback without blocking development velocity. This integration pattern---detection as a gate, remediation as a suggestion---preserves developer autonomy while ensuring no vulnerability goes unnoticed.
...
\textbf{Internal validity.} (1)~\emph{LLM non-determinism}: the detection stage uses frontier LLMs (Claude Sonnet 4.6, GPT-4o) which produce non-deterministic outputs. We mitigate this with temperature=0 and multi-run ensemble (2 runs per provider), but results may vary across API versions. (2)~\emph{Re-scan verification}: vulnerability elimination is measured by re-scanning the patched contract with the same tool that detected the original vulnerability. A false negative on re-scan would inflate the elimination rate. We partially mitigate this by running re-scan with \emph{all} detection layers, not just the original detector.

\textbf{External validity.} (3)~\emph{Corpus selection bias}: SmartBugs-curated consists primarily of contracts from 2017--2019 with known vulnerabilities. These contracts are simpler (avg.\ 85 LoC) and more heavily studied than production DeFi protocols (avg.\ 500+ LoC with complex inheritance). Results on production code may differ, particularly for the compilation rate (which depends heavily on dependency availability). (4)~\emph{Vulnerability distribution}: the corpus over-represents reentrancy (25 contracts) and unchecked low-level calls (48 contracts) relative to their frequency in production code. Categories like oracle manipulation, MEV, and flash loan attacks---which dominate real-world exploits~\cite{atzei2017survey}---are absent from SmartBugs.

\textbf{Construct validity.} (5)~\emph{Compilation $\neq$ correctness}: a compiling patch is not necessarily semantically correct. The patch could introduce subtle behavioral changes (e.g., a reentrancy guard that blocks legitimate callback patterns). Our 0\% regression rate measures only whether \emph{new} vulnerability classes are introduced, not whether all original functionality is preserved.
...
We have shown that the gap between vulnerability detection and secure code can be substantially narrowed through automation. Evaluated on 143 contracts from the SmartBugs-curated corpus, MIESC's five-stage pipeline achieves 87\% fix application rate, 63\% standalone compilation (88\% on self-contained contracts), 84\% vulnerability elimination, and zero regressions. Compared to five prior repair tools (Table~\ref{tab:comparison}), MIESC is the only system that generates exploit tests, formal specifications, and compliance documentation alongside patches.

The key insight is that each stage \emph{feeds the next}: detection informs patching, patching informs test generation, test generation closes the loop by verifying the fix, and formal verification exhaustively proves patch correctness when tractable. This compositional architecture means that improvements to any single stage automatically propagate through the pipeline.

\textbf{Limitations.} The 63\% standalone compilation rate (Section~\ref{sec:threats}) reflects the benchmark corpus composition rather than a fundamental patching limitation. The text-level patching approach cannot restructure code (e.g., apply CEI pattern) or handle cross-contract vulnerabilities. SMTChecker verification is tractable on only 16\% of contracts within the default timeout.
