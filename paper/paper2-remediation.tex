...
Table~\ref{tab:e2e} traces the full pipeline on a single illustrative contract. The complete pipeline---from scan to report---executes in under 10 seconds in local mode. For corpus-scale remediation results, see Tables~\ref{tab:fix}--\ref{tab:fix-category}. Across the 143-contract corpus, the v-next remediation baseline applies fixes to 86\% of contracts under the current scan, compiles 100\% of patched contracts standalone, eliminates target-category HIGH/CRITICAL findings in 72\%, satisfies the bounded no-regression criterion for 98\% of patched contracts, and leaves 57\% of patched contracts clean of HIGH findings under external Slither validation.
...
\textbf{Internal validity.} (1)~\emph{LLM non-determinism}: the detection stage can use frontier LLMs, whose outputs may vary across model versions and provider-side updates. The reproducible Paper~1 profile fixes the evaluated artifact and records layer selection, but future API runs may differ. (2)~\emph{Re-scan verification}: vulnerability elimination is measured by re-scanning the patched contract with the same pipeline family that detected the original vulnerability. A false negative on re-scan would inflate the elimination rate. The v-next baseline mitigates this by adding external Slither validation over every patched contract that compiles, recording 2 scan-empty retries, and reporting clean-HIGH results and residual HIGH findings separately from the MIESC re-scan rather than merging them into a single success metric.
...
\textbf{Limitations.} The v-next 100\% standalone compilation rate applies to emitted patches after current-scan filtering; it should not be read as proof that every historical SmartBugs contract requires or receives a patch. The text-level patching approach cannot restructure arbitrary code (e.g., fully general CEI refactoring) or handle cross-contract vulnerabilities. SMTChecker verification is tractable on only a minority of contracts within the default timeout, and corpus-wide semantic equivalence remains future work.
...
Taken together, these results extend MIESC beyond detection into the remediation half of the security loop: it moves automatically from a finding to inspectable patches, exploit tests, specifications, and compliance evidence, while reporting each residual failure (57\% external clean-HIGH) rather than hiding it behind a headline success rate. Closing this find$\rightarrow$fix$\rightarrow$prove loop, which remains manual in single-purpose tools, is what positions MIESC as a security framework rather than a detector or a patch generator. MIESC is available under AGPL-3.0 at \url{https://github.com/fboiero/MIESC}.
