...
These limitations represent a fundamental tradeoff: text-level patching achieves broad current-scan coverage across Solidity versions (0.4--0.8) and, in the v2 run, 100\% standalone compilation for emitted patches, while AST-level approaches like sGuard achieve higher precision but are limited to specific compiler versions.
...
We have shown that the gap between vulnerability detection and actionable remediation artifacts can be substantially narrowed through automation. Evaluated on 143 contracts from the SmartBugs-curated corpus, the v2 MIESC remediation baseline applies fixes to 123 contracts under the current scan (86\%), compiles all 123 emitted patches standalone (100\%), eliminates the target-category finding in 86/123 cases by MIESC re-scan (70\%), satisfies the bounded no-regression criterion in 121/123 cases (98\%), and reports 58/123 patched contracts clean of HIGH findings under external Slither validation (47\%). Compared to five prior repair tools (Table~\ref{tab:comparison}), MIESC is the only system in our comparison that reports exploit tests, formal specifications, and compliance documentation alongside patches.
